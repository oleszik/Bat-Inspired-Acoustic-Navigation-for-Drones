...
def _reconstruct_maps(
    ...
    # Seed occupancy mask.
    if corridor_mode:
        occ_seed = ((occ_ratio > 0.50) & (occ_obs >= 1.1)) | (occ_prob_raw > 0.62)
    else:
        occ_seed = ((occ_ratio > 0.58) & (occ_obs >= 2.0)) | (occ_prob_raw > 0.68)

    # Corridor diagnostics and long-component stats before cleanup.
    long_h_before = 0
    long_v_before = 0
    ...
    # Count preserved/removed long components.
    ...
    for cid in range(1, n_a + 1):
        ...
        if h >= long_len_thresh and w <= 4 and area >= long_len_thresh:
            ...
            long_after_mask |= comp
    if corridor_mode:
        preserved_long_component_count = int(max(0, long_h_after + long_v_after))
    ...
    if corridor_mode:
        # Detect likely top and bottom corridor wall bands from evidence rows.
        row_strength = (((occ_ratio > 0.34) & (occ_obs >= 0.55)).astype(np.float32)).mean(axis=1)
        half = ny // 2
        top_idx = int(np.argmax(row_strength[: max(2, half)]))
        bot_idx = int(half + np.argmax(row_strength[half:])) if half < ny else ny - 1
        top_s = float(row_strength[top_idx])
        bot_s = float(row_strength[bot_idx])
        if top_s > 0.03 and bot_s > 0.03 and (bot_idx - top_idx) > max(5, int(0.18 * ny)):
            top_lo, top_hi = max(0, top_idx - 2), min(ny, top_idx + 3)
            bot_lo, bot_hi = max(0, bot_idx - 2), min(ny, bot_idx + 3)
            corridor_band[top_lo:top_hi, :] = True
            corridor_band[bot_lo:bot_hi, :] = True

            # Preserve long border-parallel bands.
            preserve_top = ((occ_obs[top_lo:top_hi, :] >= 0.45) | (occ_ratio[top_lo:top_hi, :] > 0.26) | (occ_prob_raw[top_lo:top_hi, :] > 0.45))
            preserve_bot = ((occ_obs[bot_lo:bot_hi, :] >= 0.45) | (occ_ratio[bot_lo:bot_hi, :] > 0.26) | (occ_prob_raw[bot_lo:bot_hi, :] > 0.45))
            occ_mask[top_lo:top_hi, :] |= preserve_top
            occ_mask[bot_lo:bot_hi, :] |= preserve_bot

            # Keep central opening where free evidence is repeatedly observed.
            passage_rows = slice(min(top_hi, bot_lo), max(top_hi, bot_lo))
            if passage_rows.stop > passage_rows.start:
                preserved_open_mask[passage_rows, :] = (free_obs[passage_rows, :] >= 2.0) & (occ_obs[passage_rows, :] <= 0.45)
                occ_mask[preserved_open_mask] = False

            # Band-thickness recovery: normal-only local thickening.
            rec_top = np.zeros_like(occ_mask, dtype=bool)
            rec_bot = np.zeros_like(occ_mask, dtype=bool)
                ...
                for xx in range(nx):
                    ...
                    for nyy in (yy - 1, yy + 1):
                        if 0 <= nyy < ny and not occ_mask[nyy, xx]:
                            if ((occ_obs[nyy, xx] >= 0.35) or (occ_prob_raw[nyy, xx] > 0.42)) and free_obs[nyy, xx] < 2.2 and not preserved_open_mask[nyy, xx]:
                                rec_top[nyy, xx] = True
                ...
                for xx in range(nx):
                    ...
                    for nyy in (yy - 1, yy + 1):
                        if 0 <= nyy < ny and not occ_mask[nyy, xx]:
                            if ((occ_obs[nyy, xx] >= 0.35) or (occ_prob_raw[nyy, xx] > 0.42)) and free_obs[nyy, xx] < 2.2 and not preserved_open_mask[nyy, xx]:
                                rec_bot[nyy, xx] = True
            recovered_thickness_mask = rec_top | rec_bot
            occ_mask |= recovered_thickness_mask

            # Promote weak unknown cells near long bands to occupied, except preserved openings.
            near_band = _neighbor_count(corridor_band.astype(np.uint8)) >= 1
            promote = (
                near_band
                & (~occ_mask)
                & ((occ_obs >= 0.50) | (occ_prob_raw > 0.44))
                & (free_obs < 2.0)
                & (~preserved_open_mask)
            )
            promoted_occ_mask = promote
            occ_mask |= promoted_occ_mask
            ...
            if len(gt_band_rows) > 0:
                band_lengths = []
                for yb in gt_band_rows:
                    band_lengths.append(float(gt_occ[yb].mean()))
    ...
    unknown_ratio = float((~observed_mask).mean())
...
def _save_overlay_plot(
    ...
    axes[2].imshow(ep["long_wall_before_mask"], cmap="Greens", vmin=0, vmax=1)
    axes[2].set_title("Long Walls Before")
    axes[3].imshow(ep["long_wall_after_mask"], cmap="Greens", vmin=0, vmax=1)
    axes[3].set_title("Long Walls After")
    axes[4].imshow(ep["recovered_band_thickness_mask"], cmap="Oranges", vmin=0, vmax=1)
    axes[4].set_title("Recovered Band Thickness")
    axes[5].imshow(ep["promoted_unknown_to_occupied_mask"], cmap="Reds", vmin=0, vmax=1)
    axes[5].set_title("Promoted Unknown->Occ")
    axes[6].imshow(ep["preserved_open_corridor_mask"], cmap="Blues", vmin=0, vmax=1)
    axes[6].set_title("Preserved Open/Free")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Phase 2A v2.2 corridor-only acoustic mapping patch.")
    ...
    p.add_argument("--output-dir", type=str, default="runs/phase2_mapping_v22")
...
def main() -> None:
    ...
    for d in diff_names:
        ...
        for mi, map_name in enumerate(map_names, start=1):
            ...
            for ep_idx in range(1, args.episodes_per_map + 1):
                ...
                print(
                    f"[Phase2A.v2.2] map={map_name} ({mi}/{len(map_names)}) diff={d} ep={ep_idx}/{args.episodes_per_map} "
    ...
    # v1/v2/v2.1/v2.2 comparison if prior results exist
    ...
    for d in diff_names:
        if d not in agg:
            continue
        row = {
            "v22_map_accuracy": agg[d]["mean_map_accuracy"],
            "v22_wall_reconstruction_error_m": agg[d]["mean_wall_reconstruction_error_m"],
            "v22_doorway_precision": agg[d]["mean_doorway_precision"],
            "v22_doorway_recall": agg[d]["mean_doorway_recall"],
            "v22_localization_rmse_m": agg[d]["mean_localization_rmse_m"],
            "v22_collision_rate": agg[d]["mean_collision_rate"],
            "v22_timeout_rate": agg[d]["mean_timeout_rate"],
        }
        ...
        comparison_v1_v2_v21_v22[d] = row

    clean_corridor_doorway = {}
    try:
        clean_corridor_doorway = {
            "v22_clean_corridor_map_accuracy": all_results["clean"]["corridor"]["map_accuracy"],
            "v22_clean_doorway_map_accuracy": all_results["clean"]["doorway"]["map_accuracy"],
        }
    ...
    results = {
        "phase": "Phase 2A v2.2 corridor-only acoustic mapping patch",
        ...
        "comparison_v1_v2_v21_v22": comparison_v1_v2_v21_v22,
