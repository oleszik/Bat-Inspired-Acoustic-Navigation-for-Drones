...
def _reconstruct_maps(
    ...
    if map_name == "corridor":
        occ_mask = ((occ_ratio > 0.52) & (occ_obs >= 1.4)) | (occ_prob_raw > 0.63)
    else:
        occ_mask = ((occ_ratio > 0.58) & (occ_obs >= 2.0)) | (occ_prob_raw > 0.68)

    neigh = _neighbor_count(occ_mask.astype(np.uint8))
    occ_mask = occ_mask & (neigh >= (1 if map_name == "corridor" else 2))
    occ_mask, comp_diag, preserved_long = _remove_small_components_with_diag(
        occ_mask,
        min_area=4,
        preserve_long=(map_name == "corridor"),
        long_len_thresh=max(10, int(round(nx * 0.35))),
    )
    removed_components_mask = ((~occ_mask) & (((occ_ratio > 0.52) & (occ_obs >= 1.0)) | (occ_prob_raw > 0.60)))
    ...
    if map_name == "corridor":
        row_strength = (((occ_ratio > 0.40) & (occ_obs >= 0.8)).astype(np.float32)).mean(axis=1)
        half = ny // 2
        ...
        if top_s > 0.06 and bot_s > 0.06 and (bot_idx - top_idx) > max(5, int(0.20 * ny)):
            ...
            # Preserve long corridor borders and continuity across x.
            occ_mask[top_lo:top_hi, :] |= ((occ_obs[top_lo:top_hi, :] >= 0.6) | (occ_ratio[top_lo:top_hi, :] > 0.34))
            occ_mask[bot_lo:bot_hi, :] |= ((occ_obs[bot_lo:bot_hi, :] >= 0.6) | (occ_ratio[bot_lo:bot_hi, :] > 0.34))
            for yy in range(top_lo, top_hi):
                if float(occ_mask[yy].mean()) > 0.12:
                    occ_mask[yy, :] = True
    ...
    stable_occ = occ_mask & (occ_obs >= (1.4 if map_name == "corridor" else 2.0))
    ...
    wall_probability[~wall_candidate] *= 0.25 if map_name == "corridor" else 0.3
    ...
    if map_name == "corridor" and corridor_band.any():
        corridor_cont_score = _corridor_wall_continuity_score(wall_mask, corridor_band)
...
def _save_overlay_plot(
    ...
    axes[2].imshow(ep["removed_components_mask"], cmap="Reds", vmin=0, vmax=1)
    axes[2].set_title("Removed Components")
    axes[3].imshow(ep["preserved_long_walls_mask"], cmap="Greens", vmin=0, vmax=1)
    axes[3].set_title("Preserved Long Walls")
    axes[4].imshow(ep["raw_doorway_candidate_map"], cmap="plasma")
    axes[4].set_title("Raw Doorway Cands")
    # wall segments overlay
    axes[5].imshow(ep["wall_probability"], cmap="magma", vmin=0, vmax=1, alpha=0.9)
    for s in ep["wall_segments"]:
        axes[5].plot([s["x0"], s["x1"]], [s["y0"], s["y1"]], "w-", linewidth=1.0)
    axes[5].set_title("Wall Prob + Segments")
    # accepted doorway candidates
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Phase 2A v2.1 targeted acoustic mapping patch.")
    ...
    p.add_argument("--output-dir", type=str, default="runs/phase2_mapping_v21")
...
def main() -> None:
    ...
    for d in diff_names:
        ...
        for mi, map_name in enumerate(map_names, start=1):
            ...
            for ep_idx in range(1, args.episodes_per_map + 1):
                ...
                print(
                    f"[Phase2A.v2.1] map={map_name} ({mi}/{len(map_names)}) diff={d} ep={ep_idx}/{args.episodes_per_map} "
    ...
    # v1/v2/v2.1 comparison if prior results exist
    ...
    for d in diff_names:
        if d not in agg:
            continue
        row = {
            "v21_map_accuracy": agg[d]["mean_map_accuracy"],
            "v21_wall_reconstruction_error_m": agg[d]["mean_wall_reconstruction_error_m"],
            "v21_doorway_precision": agg[d]["mean_doorway_precision"],
            "v21_doorway_recall": agg[d]["mean_doorway_recall"],
            "v21_localization_rmse_m": agg[d]["mean_localization_rmse_m"],
            "v21_collision_rate": agg[d]["mean_collision_rate"],
            "v21_timeout_rate": agg[d]["mean_timeout_rate"],
        }
        ...
        comparison_v1_v2_v21[d] = row

    clean_corridor_doorway = {}
    try:
        clean_corridor_doorway = {
            "v21_clean_corridor_map_accuracy": all_results["clean"]["corridor"]["map_accuracy"],
            "v21_clean_doorway_map_accuracy": all_results["clean"]["doorway"]["map_accuracy"],
        }
    ...
    results = {
        "phase": "Phase 2A v2.1 targeted acoustic mapping patch",
        ...
        "comparison_v1_v2_v21": comparison_v1_v2_v21,
