"""
Simple 2D acoustic navigation simulator v2.

Upgrades over v1:
- Goal-directed heading bias
- Short-term front-group memory
- Stuck detection
- Scan-turn recovery
...
RESULT_JSON = RESULT_DIR / "simple_2d_acoustic_nav_v2_results.json"
RESULT_PATHS_PNG = RESULT_DIR / "simple_2d_acoustic_nav_v2_paths.png"
RESULT_ACTION_PNG = RESULT_DIR / "simple_2d_acoustic_nav_v2_action_distribution.png"
...
def synthesize_sector_perception(true_d: float, rng: np.random.Generator) -> Dict[str, float | str | int]:
    """
    Conservative, configurable perception:
    close obstacles -> OBSTACLE likely,
    far/weak -> UNCERTAIN likely.
    """
    if true_d < 0.45:
...
def predict_forward_collision(
    m: MapDef, x: float, y: float, heading: float, dist: float, margin: float
) -> bool:
...
def decide_action_v2(
    sec: Dict[str, Dict[str, float | str | int]],
    true_dist: Dict[str, float],
    heading_error_rad: float,
    consecutive_front_clear_count: int,
    consecutive_front_blocked_count: int,
    stuck: bool,
    stuck_steps: int,
) -> Tuple[str, float | None, bool]:
    """
    Returns (action, forced_turn_deg, used_stuck_recovery).
    """
    ...
    # Hard rule: no forward if any front-group obstacle predicted.
    if any_front_obstacle:
        if f == "OBSTACLE":
            if (l == "CLEAR") and (r != "CLEAR"):
                return ACTION_LEFT, None, False
            if (r == "CLEAR") and (l != "CLEAR"):
                return ACTION_RIGHT, None, False
            if (l == "CLEAR") and (r == "CLEAR"):
                return choose_clearer_side_by_true_distance(true_dist), None, False
            return ACTION_STOP, None, False
        # side-front obstacle with front not obstacle
        if (fl == "OBSTACLE") and (fr == "CLEAR"):
            return ACTION_RIGHT, None, False
        if (fr == "OBSTACLE") and (fl == "CLEAR"):
            return ACTION_LEFT, None, False
        return ACTION_RESAMPLE, None, False

    # Stuck recovery block.
    if stuck:
        turn_to_goal = ACTION_LEFT if heading_error_rad > 0 else ACTION_RIGHT
        if stuck_steps >= 10:
            # Force wider scan turn.
            if f != "OBSTACLE":
                return turn_to_goal, 30.0, True
            return choose_clearer_side_by_true_distance(true_dist), 30.0, True
        # Shorter stuck turns.
        if f != "OBSTACLE":
            return turn_to_goal, None, True
        return choose_clearer_side_by_true_distance(true_dist), None, True

    # Goal-directed steering bias.
    abs_err_deg = abs(math.degrees(heading_error_rad))
    if abs_err_deg > 45.0:
        return (ACTION_LEFT if heading_error_rad > 0 else ACTION_RIGHT), None, False
    if abs_err_deg > 20.0 and front_not_clearly_open:
        return (ACTION_LEFT if heading_error_rad > 0 else ACTION_RIGHT), None, False

    # Memory-based forward relaxation.
    if front_all_clear and consecutive_front_clear_count >= 4:
        return ACTION_FAST, None, False
    if front_all_clear and consecutive_front_clear_count >= 2:
        return ACTION_SLOW, None, False
    if (f == "CLEAR") and (fl != "OBSTACLE") and (fr != "OBSTACLE"):
        return ACTION_PROBE, None, False

    if consecutive_front_blocked_count >= 3:
        return choose_clearer_side_by_true_distance(true_dist), None, False
    return ACTION_RESAMPLE, None, False
...
def run_episode(m: MapDef, rng: np.random.Generator) -> Dict[str, object]:
    ...
    goal_dist_hist = deque(maxlen=20)
    ...
    for step in range(1, MAX_STEPS + 1):
        ...
        # Stuck: no improvement >=0.05m across last 20 steps.
        stuck = False
        if len(goal_dist_hist) >= 20:
            if (goal_dist_hist[0] - goal_dist_hist[-1]) < 0.05:
                stuck = True

        if len(goal_dist_hist) >= 2 and (goal_dist_hist[-2] - goal_dist_hist[-1]) < 1e-3:
        ...
        action, forced_turn_deg, used_stuck = decide_action_v2(
            sec=sec,
            true_dist=true_dist,
            heading_error_rad=heading_error,
            consecutive_front_clear_count=consecutive_front_clear_count,
            consecutive_front_blocked_count=consecutive_front_blocked_count,
            stuck=stuck,
        ...
        if action in {ACTION_FAST, ACTION_SLOW, ACTION_PROBE}:
            ...
            if predict_forward_collision(m, x, y, heading, step_dist, SAFETY_MARGIN):
                # Prefer turn to clearer side; else stop/reverse if reverse path is safe.
...
def main() -> None:
    ...
    v1_summary = None
    if V1_JSON.exists():
        v1_summary = json.loads(V1_JSON.read_text(encoding="utf-8"))
    ...
    ax.set_title("Simple 2D Acoustic Nav v2: Action Distribution")
    ...
    print("\nV2 Overall action distribution:")
    for a in ALL_ACTIONS:
        print(f"  {a}: {action_distribution[a]['count']} ({100*action_distribution[a]['rate']:.2f}%)")
    print(f"\nV2 total stuck recovery actions: {total_stuck_recoveries}")

    if v1_summary is not None and "maps" in v1_summary:
        print("\nV1 vs V2 comparison:")
        for m in maps:
            n = m.name
            if n in v1_summary["maps"]:
                a = v1_summary["maps"][n]
                b = all_results[n]
                print(
