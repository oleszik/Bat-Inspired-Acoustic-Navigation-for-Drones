"""
Simple 2D acoustic navigation simulator v4.

Adds on top of v3:
- Local planner over candidate actions
- Adaptive turn angles (10/20/30 deg by heading error and proximity)
- Probe-to-commit forward mode
"""
...
RESULT_JSON = RESULT_DIR / "simple_2d_acoustic_nav_v4_results.json"
RESULT_PATHS_PNG = RESULT_DIR / "simple_2d_acoustic_nav_v4_paths.png"
RESULT_ACTION_PNG = RESULT_DIR / "simple_2d_acoustic_nav_v4_action_distribution.png"
...
def run_episode(m: MapDef, rng: np.random.Generator) -> Dict[str, object]:
    ...
    for step in range(1, MAX_STEPS + 1):
                ...
                if commit_slow_remaining > 0 and (f == "CLEAR") and (fl != "OBSTACLE") and (fr != "OBSTACLE"):
                    ...
                    if abs_err_deg > 45.0:
                        ...
                        if (
                            ...
                            commit_slow_remaining = COMMIT_SLOW_STEPS
                            candidates = [ACTION_SLOW, ACTION_PROBE]
        ...
        if candidates and candidates[0] == ACTION_RESAMPLE and consecutive_resample_count >= RESAMPLE_CAP:
        ...
        for c in safe_candidates:
            score = local_plan_score(m, x, y, heading, gx, gy, c, adaptive_turn)
            if score > best_score:
                best_score = score
                best_action = c
        action = best_action
...
def main() -> None:
    ...
    ax.set_title("Simple 2D Acoustic Nav v4: Action Distribution")
    ...
    print("\nV4 Overall action distribution:")
    for a in ALL_ACTIONS:
        print(f"  {a}: {action_distribution[a]['count']} ({100*action_distribution[a]['rate']:.2f}%)")
    print(f"\nV4 total stuck recovery actions: {total_stuck_recoveries}")
    print(f"V4 total 30-degree scan turns: {total_scan_turns}")
