...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Phase 2D.3b mapper-guided navigation v3.1 targeted smoothing.")
    ...
    parser.add_argument("--output-dir", type=str, default="runs/phase2_mapper_guided_navigation_v31")
    ...
    parser.add_argument("--min-frontier-commit-steps", type=int, default=10)
    ...
    for y in range(1, ny - 1):
        for x in range(1, nx - 1):
            ...
            if min_conf < 0.45:
                ...
                if coverage_flat:
                    score += float(0.35 * info_gain + 0.25 * frontier_area)
                frontier.append(
                    {
                        "score": score,
                        "x": x,
...
def run_episode(
    ...
    max_consecutive_probe_steps = 2
    probe_cooldown_steps = 5
    consecutive_probe_steps = 0
    consecutive_resample_steps = 0
    probe_cooldown = 0
    probe_suppressed_count = 0
    probe_cooldown_active_count = 0
    consecutive_probe_max_observed = 0
    safe_forward_candidate_count = 0
    safe_forward_chosen_count = 0
    safe_forward_blocked_count = 0
    safe_forward_coverage_gains: List[float] = []
    ...
    committed_frontier_steps: List[int] = []
    current_committed_frontier: Optional[Tuple[int, int]] = None
    current_frontier_commit_age = 0
    ...
    for step in range(max_steps):
        ...
        committed_switch_reason = "hold"
        if current_committed_frontier is None:
            current_committed_frontier = proposed_frontier
            current_frontier_commit_age = 0
            committed_switch_reason = "init"
        elif current_committed_frontier is not None:
            tx, ty = current_committed_frontier
            wx = (tx + 0.5) * cell_size
            wy = (ty + 0.5) * cell_size
            dist_to_committed = float(math.hypot(wx - x, wy - y))
            committed_blocked = bool(
                float(obs["front"]["distance"]) < 0.12
                and predict_collision(m, x, y, heading, 0.03, 0.12, 0.05 if m.name != "cluttered_room" else 0.08)
            )
            committed_reached = dist_to_committed < 0.22
            target_changed = proposed_frontier is not None and proposed_frontier != current_committed_frontier
            can_switch_frontier = current_frontier_commit_age >= min_frontier_commit_steps
            if proposed_frontier is None:
                committed_switch_reason = "hold_no_proposal"
            elif target_changed and (can_switch_frontier or committed_blocked or committed_reached or stagnation):
                committed_frontier_steps.append(current_frontier_commit_age)
                if not (committed_blocked or committed_reached or stagnation):
                    premature_frontier_switch_count += 1
                current_committed_frontier = proposed_frontier
                current_frontier_commit_age = 0
                committed_switch_reason = (
                    "blocked" if committed_blocked else ("reached" if committed_reached else ("stagnation" if stagnation else "commit_elapsed"))
                )
            else:
                committed_switch_reason = "commit_hold"
        frontier = current_committed_frontier
        current_frontier_commit_age += 1 if frontier is not None else 0
        ...
        expected_forward_gain = float(max(0.0, 1.0 - ahead_conf) + (0.35 if ahead_unobserved else 0.0))
        forward_safe_slow = not predict_collision(m, x, y, heading, 0.08, 0.12, 0.05 if m.name != "cluttered_room" else 0.08)
        forward_candidate = (
            forward_safe_slow
            and float(obs["front"]["distance"]) > 0.22
            and front_free > 0.54
            and front_occ < 0.68
            and expected_forward_gain > 0.28
        )
        if forward_candidate:
            safe_forward_candidate_count += 1
            safe_forward_coverage_gains.append(expected_forward_gain)
            safe_forward_chosen_recorded = False
            if action == "PROBE_FORWARD" and mode not in {"emergency_avoidance", "doorway_approach"}:
                action = "MOVE_FORWARD_SLOW"
                safe_forward_chosen_count += 1
                safe_forward_chosen_recorded = True
        elif action in {"PROBE_FORWARD", "TURN_LEFT", "TURN_RIGHT"} and not forward_safe_slow:
            safe_forward_blocked_count += 1
            safe_forward_chosen_recorded = False
        else:
            safe_forward_chosen_recorded = False
        ...
        if forward_candidate and action == "MOVE_FORWARD_SLOW" and not safe_forward_chosen_recorded:
            safe_forward_chosen_count += 1
            safe_forward_chosen_recorded = True
        ...
        if action == "PROBE_FORWARD":
            ...
            consecutive_probe_max_observed = max(consecutive_probe_max_observed, consecutive_probe_steps)
            probe_cooldown = 0
        else:
    ...
    return {
        ...
        "premature_frontier_switch_count": int(premature_frontier_switch_count),
        "committed_frontier_steps_mean": float(
            np.mean(committed_frontier_steps + ([current_frontier_commit_age] if current_frontier_commit_age > 0 else []))
        )
        if (committed_frontier_steps or current_frontier_commit_age > 0)
        else 0.0,
        ...
        "probe_suppressed_count": int(probe_suppressed_count),
        "probe_cooldown_active_count": int(probe_cooldown_active_count),
        "consecutive_probe_max_observed": int(consecutive_probe_max_observed),
        "safe_forward_candidate_count": int(safe_forward_candidate_count),
        "safe_forward_chosen_count": int(safe_forward_chosen_count),
        "safe_forward_blocked_count": int(safe_forward_blocked_count),
        "safe_forward_coverage_gain_mean": float(np.mean(safe_forward_coverage_gains)) if safe_forward_coverage_gains else 0.0,
...
def aggregate(episodes: List[Dict[str, object]]) -> Dict[str, object]:
    ...
    return {
        ...
        "premature_frontier_switch_count": int(np.sum([e.get("premature_frontier_switch_count", 0) for e in episodes])),
        "committed_frontier_steps_mean": float(np.mean([e.get("committed_frontier_steps_mean", 0.0) for e in episodes])),
        ...
        "probe_suppressed_count": int(np.sum([e.get("probe_suppressed_count", 0) for e in episodes])),
        "probe_cooldown_active_count": int(np.sum([e.get("probe_cooldown_active_count", 0) for e in episodes])),
        "consecutive_probe_max_observed": int(np.max([e.get("consecutive_probe_max_observed", 0) for e in episodes])),
        "safe_forward_candidate_count": int(np.sum([e.get("safe_forward_candidate_count", 0) for e in episodes])),
        "safe_forward_chosen_count": int(np.sum([e.get("safe_forward_chosen_count", 0) for e in episodes])),
        "safe_forward_blocked_count": int(np.sum([e.get("safe_forward_blocked_count", 0) for e in episodes])),
        "safe_forward_coverage_gain_mean": float(np.mean([e.get("safe_forward_coverage_gain_mean", 0.0) for e in episodes])),
...
def main() -> None:
    ...
    results: Dict[str, object] = {
        "config": {
            ...
            "min_frontier_commit_steps": args.min_frontier_commit_steps,
    ...
    # Explicit v1/v2/v3/v4/v3.1 comparison table.
    version_paths = {
    ...
    version_comp: Dict[str, object] = {"v31": {k: float(overall.get(k, 0.0)) for k in comparison_keys}}
    ...
    results["comparison_v1_v2_v3_v4_v31"] = version_comp
