...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=str, default="runs/phase2_mapper_guided_navigation_v3_coverage_expansion")
    parser.add_argument("--target-coverage", type=float, default=0.65)
    parser.add_argument("--frontier-commit-min", type=int, default=40)
    parser.add_argument("--frontier-commit-max", type=int, default=70)
    parser.add_argument("--stagnation-window", type=int, default=50)
    parser.add_argument("--stagnation-min-gain", type=float, default=0.01)
...
def find_frontier_target(
    ...
    recent_frontier_targets: List[Tuple[int, int]],
    blocked: Optional[set] = None,
) -> Tuple[Optional[Tuple[int, int]], Dict[str, float], List[Tuple[int, int]], List[Tuple[int, int]]]:
    # Build frontier cells first, then cluster and rank clusters for coverage expansion.
    blocked = blocked or set()
    ny, nx = free_prob.shape
    frontier_mask = np.zeros((ny, nx), dtype=bool)
    ...
    for y in range(1, ny - 1):
        for x in range(1, nx - 1):
            ...
            if min(neigh_conf) < 0.45:
                frontier_mask[y, x] = True
                low_conf_count += 1

    # 4-neighbor connected component clustering for frontier cells.
        ...
        for x in range(1, nx - 1):
            ...
            while q:
                ...
                for nx2, ny2 in ((cx - 1, cy), (cx + 1, cy), (cx, cy - 1), (cx, cy + 1)):
                    if 0 <= nx2 < nx and 0 <= ny2 < ny and frontier_mask[ny2, nx2] and not visited[ny2, nx2]:
    ...
    if not clusters:
        stats = {
            ...
            "low_confidence_frontier_count": float(low_conf_count),
            "rejected_frontier_count": float(len(rejected)),
            "average_frontier_area": 0.0,
        }
        return None, stats, rejected, []

    cur_x, cur_y = current_cell
    rec_cell_set = set(recent_cells)
    rec_target_set = set(recent_frontier_targets)
    scored = []
    centers = []
    for comp in clusters:
        ...
        local_conf = float(np.mean([confidence[py, px] for px, py in comp]))
        local_free = float(np.mean([free_prob[py, px] for px, py in comp]))
        local_wall = float(np.mean([wall_prob[py, px] for px, py in comp]))
        local_door = float(np.mean([door_prob[py, px] for px, py in comp]))
        area = float(len(comp))
        # unknown-cell gain proxy around centroid
        y0, y1 = max(0, cy - 3), min(ny, cy + 4)
        x0, x1 = max(0, cx - 3), min(nx, cx + 4)
        info_gain = float(np.logical_and(observed[y0:y1, x0:x1], confidence[y0:y1, x0:x1] < 0.45).sum())
        dist = float(math.hypot(cx - cur_x, cy - cur_y))
        dist_cost = dist / 18.0
        wall_risk = max(0.0, local_wall - 0.45)
        # fake-doorway risk: high doorway prob with weak wall support
        fake_doorway_risk = local_door * max(0.0, 0.35 - local_wall)
        revisit_penalty = 0.8 if (cx, cy) in rec_target_set else 0.0
        revisit_penalty += 0.6 if (cx, cy) in rec_cell_set else 0.0
        heading_to = math.atan2(cy - cur_y, cx - cur_x)
        heading_err = abs(wrap_angle(heading_to - heading))
        heading_bonus = max(0.0, 1.0 - heading_err / math.pi)
        reachability = float(np.clip(0.65 * local_free + 0.35 * (1.0 - local_conf), 0.0, 1.0))
        score = float(
            2.8 * info_gain
            + 1.7 * area
            + 1.1 * heading_bonus
    ...
    if not scored:
        ...
        return None, stats, rejected, centers
    ...
    stats = {
        ...
        "low_confidence_frontier_count": float(low_conf_count),
        "rejected_frontier_count": float(len(rejected)),
        "average_frontier_area": float(np.mean([d["area"] for d in scored])),
    }
    return (int(best["cx"]), int(best["cy"])), stats, rejected, centers
...
def cast_observation_footprint(
    ...
    spread = np.linspace(-math.radians(16.0), math.radians(16.0), max(1, rays_per_sector))
    ray_angles = []
...
def run_episode(
    ...
    frontier_commit_min: int,
    frontier_commit_max: int,
    stagnation_window: int,
    stagnation_min_gain: float,
    ...
    rejected_frontiers_plot: List[Tuple[float, float]] = []
    ...
    recent_frontier_targets = deque(maxlen=24)
    last_frontier_cell: Optional[Tuple[int, int]] = None
    steps_since_frontier_progress = 0
    blocked_frontier_cells: set = set()
    committed_frontier: Optional[Tuple[int, int]] = None
    commit_remaining = 0
    frontier_commit_success_count = 0
    stagnation_recovery_count = 0
    safe_forward_bias_count = 0
    unknown_frontier_target_steps = 0
    high_confidence_revisit_count = 0
    local_loop_count = 0
    mean_frontier_distance_trace: List[float] = []
    frontier_score_trace: List[float] = []
    frontier_info_gain_trace: List[float] = []
    frontier_reachability_trace: List[float] = []
    rejected_frontier_trace: List[float] = []
    frontier_area_trace: List[float] = []
    ...
    for step in range(max_steps):
        ...
        observed_range = 4.0 if difficulty in {"clean", "mild_noise"} else 3.6
        rays_per_sector = 19 if difficulty in {"clean", "mild_noise"} else 15
        ...
        if len(recent_cells) >= 30:
            if len(set(list(recent_cells)[-30:])) <= 10:
                local_loop_count += 1
        ...
        window_gain = 0.0
        if len(coverage_trace) > stagnation_window:
            window_gain = float(coverage_trace[-1] - coverage_trace[-1 - stagnation_window])
        stagnation = (steps_since_frontier_progress >= stagnation_window) or (
            len(coverage_trace) > stagnation_window and window_gain < stagnation_min_gain
        )
        if stagnation:
        ...
        frontier, frontier_diag, rejected_cells, _frontier_centers = find_frontier_target(
            free_prob=free_global,
            confidence=conf_global,
            wall_prob=wall_global,
            door_prob=door_global,
            observed=observed,
            current_cell=current_cell,
            heading=heading,
            recent_cells=list(recent_cells),
            recent_frontier_targets=list(recent_frontier_targets),
            blocked=blocked_frontier_cells,
        )
        frontier_score_trace.append(float(frontier_diag["selected_frontier_score"]))
        frontier_info_gain_trace.append(float(frontier_diag["frontier_information_gain"]))
        frontier_reachability_trace.append(float(frontier_diag["frontier_reachability_score"]))
        rejected_frontier_trace.append(float(frontier_diag["rejected_frontier_count"]))
        frontier_area_trace.append(float(frontier_diag["average_frontier_area"]))
        for rx, ry in rejected_cells[:16]:
            rejected_frontiers_plot.append(((rx + 0.5) * cell_size, (ry + 0.5) * cell_size))
        target_angle = None
        if frontier is not None:
            # Frontier commitment to avoid switching every few steps.
            if (committed_frontier is None) or (commit_remaining <= 0) or stagnation:
                committed_frontier = frontier
                commit_remaining = int(rng.integers(frontier_commit_min, frontier_commit_max + 1))
                frontier_target_count += 1
                recent_frontier_targets.append(committed_frontier)
                if last_frontier_cell is not None and committed_frontier != last_frontier_cell:
            ...
            mean_frontier_distance_trace.append(float(math.hypot(fx - current_cell[0], fy - current_cell[1])))
            if float(conf_global[fy, fx]) < 0.55:
                unknown_frontier_target_steps += 1
            last_frontier_cell = (fx, fy)
            frontier_targets.append((wx, wy))
            commit_remaining = max(0, commit_remaining - 1)
        else:
        ...
        action = planner_action(
            ...
            max_consecutive_probe_steps=max_consecutive_probe_steps,
            stagnation=stagnation,
        )

        # Safe forward exploration bias toward unknown space.
        unknown_ahead = float(np.mean(conf_p[masks["front"]] < 0.48))
        if (action in {"PROBE_FORWARD", "SLOW_DOWN_AND_RESAMPLE"}) and (unknown_ahead > 0.28) and (front_free > 0.56):
            if (float(obs["front"]["distance"]) > 0.24) and (
                not predict_collision(m, x, y, heading, 0.08, 0.12, 0.05 if m.name != "cluttered_room" else 0.08)
            ):
                action = "MOVE_FORWARD_SLOW"
                safe_forward_bias_count += 1

        # Stagnation recovery sequence.
        if stagnation:
            stagnation_recovery_count += 1
            if (float(obs["front"]["distance"]) > 0.22) and (front_free > 0.50) and (
                not predict_collision(m, x, y, heading, 0.08, 0.12, 0.05 if m.name != "cluttered_room" else 0.08)
            ):
                action = "MOVE_FORWARD_SLOW"
            elif (float(obs["front"]["distance"]) > 0.14) and (
                not predict_collision(m, x, y, heading, 0.03, 0.12, 0.05 if m.name != "cluttered_room" else 0.08)
            ):
        ...
        if action == "PROBE_FORWARD":
            known_space_here = float(conf_global[current_cell[1], current_cell[0]]) > 0.72
            if known_space_here and (unknown_ahead < 0.15):
        ...
        if stagnation and last_frontier_cell is not None and coverage_now <= previous_coverage + 0.0005:
            blocked_frontier_cells.add(last_frontier_cell)
        if (not stagnation) and (coverage_now > previous_coverage + 0.003):
            frontier_commit_success_count += 1
        # If progress is flat late in the episode, stop and mark as low-coverage instead of max-step timeout.
        if step >= int(0.75 * max_steps) and stagnation and coverage_now < target_coverage:
            if len(coverage_trace) > 100 and (coverage_trace[-1] - coverage_trace[-100]) < 0.005:
                break
    ...
    occ_thresholds = [round(t, 2) for t in np.linspace(0.35, 0.75, 17)]
    occ_best_t = 0.50
    occ_best_acc = -1.0
    occ_pred = occ_global >= occ_best_t
    for ot in occ_thresholds:
        pred = occ_global >= ot
        acc = float((pred == gt_occ).mean())
        if acc > occ_best_acc:
            occ_best_acc = acc
            occ_best_t = float(ot)
            occ_pred = pred
    ...
    success = (not collision) and (coverage >= target_coverage) and ((m.name != "doorway") or crossed_doorway)
    if collision:
        failure_reason = "collision"
    elif not success and timeout:
        failure_reason = "timeout"
    elif not success and coverage < target_coverage:
    ...
    return {
        ...
        "stagnation_recovery_count": int(stagnation_recovery_count),
        ...
        "revisit_rate": float(repeated_cell_count / max(1, len(path))),
        "high_confidence_revisit_ratio": float(high_confidence_revisit_count / max(1, len(path))),
        "local_loop_count": int(local_loop_count),
        "probe_action_ratio": probe_action_ratio,
        "move_forward_ratio": move_forward_ratio,
        "turn_ratio": turn_ratio,
        "mean_distance_to_selected_frontier": float(np.mean(mean_frontier_distance_trace)) if mean_frontier_distance_trace else 0.0,
        "percent_steps_targeting_unknown_frontier": float(unknown_frontier_target_steps / max(1, len(path) - 1)),
        "percent_steps_safe_forward_bias_active": float(safe_forward_bias_count / max(1, len(path) - 1)),
        "selected_frontier_score": float(np.mean(frontier_score_trace)) if frontier_score_trace else 0.0,
        "frontier_information_gain": float(np.mean(frontier_info_gain_trace)) if frontier_info_gain_trace else 0.0,
        "frontier_reachability_score": float(np.mean(frontier_reachability_trace)) if frontier_reachability_trace else 0.0,
        "rejected_frontier_count": float(np.mean(rejected_frontier_trace)) if rejected_frontier_trace else 0.0,
        "average_frontier_area": float(np.mean(frontier_area_trace)) if frontier_area_trace else 0.0,
        "wall_threshold_used": float(wall_best_t),
        "doorway_threshold_used": float(door_best_t),
        "occupancy_threshold_used": float(occ_best_t),
        ...
        "rejected_frontiers": rejected_frontiers_plot,
...
def save_episode_plot(out_dir: Path, map_name: str, difficulty: str, ep: Dict[str, object], m) -> None:
    ...
    ax[6].imshow(ep["gt_occ"], cmap="gray_r", alpha=0.35)
    p = np.asarray(ep["path"], dtype=float)
    if len(p) > 1:
        ax[6].plot(p[:, 0] / 0.25, p[:, 1] / 0.25, color="tab:blue", linewidth=1.5, label="trajectory")
    if ep["frontier_targets"]:
        ft = np.asarray(ep["frontier_targets"], dtype=float)
        ax[6].scatter(ft[:, 0] / 0.25, ft[:, 1] / 0.25, s=8, c="gold", label="frontiers", alpha=0.65)
    if ep.get("rejected_frontiers"):
        rf = np.asarray(ep["rejected_frontiers"], dtype=float)
        if len(rf) > 0:
            ax[6].scatter(rf[:, 0] / 0.25, rf[:, 1] / 0.25, s=6, c="gray", marker=".", label="rejected")
    if ep["accepted_doorways"]:
        da = np.asarray(ep["accepted_doorways"], dtype=float)
        ax[6].scatter(da[:, 0] / 0.25, da[:, 1] / 0.25, s=14, c="lime", marker="o", label="doorway accepted")
    if ep["rejected_doorways"]:
        dr = np.asarray(ep["rejected_doorways"], dtype=float)
        ax[6].scatter(dr[:, 0] / 0.25, dr[:, 1] / 0.25, s=14, c="red", marker="x", label="doorway rejected")
    if ep.get("stagnation_points"):
        sp = np.asarray(ep["stagnation_points"], dtype=float)
        ax[6].scatter(sp[:, 0] / 0.25, sp[:, 1] / 0.25, s=10, c="orange", marker="s", label="stagnation")
    ax[6].set_title("Trajectory + Frontiers + Doorway Decisions")
    ax[6].legend(fontsize=7, loc="upper right")

    ax[7].imshow(ep.get("door_map_thresholded", np.zeros_like(ep["gt_door"])), cmap="gray_r", vmin=0, vmax=1)
    ax[7].set_title(f"Thresholded Doorway (t={ep.get('doorway_threshold_used', 0.0):.2f})")
...
def aggregate(episodes: List[Dict[str, object]]) -> Dict[str, object]:
    ...
    return {
        ...
        "stagnation_recovery_count": int(np.sum([e.get("stagnation_recovery_count", 0) for e in episodes])),
        ...
        "frontier_commit_success_count": int(np.sum([e.get("frontier_commit_success_count", 0) for e in episodes])),
        "repeated_cell_ratio": float(np.mean([e.get("repeated_cell_ratio", 0.0) for e in episodes])),
        "revisit_rate": float(np.mean([e.get("revisit_rate", 0.0) for e in episodes])),
        ...
        "mean_distance_to_selected_frontier": float(np.mean([e.get("mean_distance_to_selected_frontier", 0.0) for e in episodes])),
        "percent_steps_targeting_unknown_frontier": float(np.mean([e.get("percent_steps_targeting_unknown_frontier", 0.0) for e in episodes])),
        "percent_steps_safe_forward_bias_active": float(np.mean([e.get("percent_steps_safe_forward_bias_active", 0.0) for e in episodes])),
...
def main() -> None:
    ...
    results: Dict[str, object] = {
        "config": {
            ...
            "frontier_commit_min": args.frontier_commit_min,
            "frontier_commit_max": args.frontier_commit_max,
            "stagnation_window": args.stagnation_window,
            "stagnation_min_gain": args.stagnation_min_gain,
    ...
    out_json = out_dir / "mapper_guided_navigation_v3_results.json"
    out_json.write_text(json.dumps(results, indent=2), encoding="utf-8")

    # Required additional reports.
    v2_path = Path("runs/phase2_mapper_guided_navigation_v2/mapper_guided_navigation_results.json")
    comparison_v2_v3: Dict[str, object] = {"v3_overall": overall}
    if v2_path.exists():
        v2_obj = json.loads(v2_path.read_text(encoding="utf-8"))
        v2_ov = v2_obj.get("overall", {}) if isinstance(v2_obj, dict) else {}
        comparison_v2_v3["v2_overall"] = v2_ov
        if isinstance(v2_ov, dict):
            metrics = [
                "success_rate",
                "collision_rate",
                "fake_doorway_approach_rate",
            ...
            comparison_v2_v3["delta_v3_minus_v2"] = {
                m: float(overall.get(m, 0.0) - float(v2_ov.get(m, 0.0))) for m in metrics
            }
    (out_dir / "comparison_v2_vs_v3.json").write_text(json.dumps(comparison_v2_v3, indent=2), encoding="utf-8")

    coverage_by_map_and_difficulty: Dict[str, object] = {}
    for diff_name, diff_obj in by_diff.items():
        maps_obj = diff_obj.get("maps", {}) if isinstance(diff_obj, dict) else {}
        coverage_by_map_and_difficulty[diff_name] = {}
        if isinstance(maps_obj, dict):
            for map_name, map_metrics in maps_obj.items():
                if isinstance(map_metrics, dict):
                    coverage_by_map_and_difficulty[diff_name][map_name] = {
                        "coverage": float(map_metrics.get("exploration_coverage", 0.0)),
                        "success_rate": float(map_metrics.get("success_rate", 0.0)),
                        "collision_rate": float(map_metrics.get("collision_rate", 0.0)),
                        "timeout_rate": float(map_metrics.get("timeout_rate", 0.0)),
                    }
    (out_dir / "coverage_by_map_and_difficulty.json").write_text(
        json.dumps(coverage_by_map_and_difficulty, indent=2), encoding="utf-8"
    )

    planner_diagnostics = {
        "overall": {
            "coverage_gain_per_100_steps": float(overall.get("coverage_gain_per_100_steps", 0.0)),
            "frontier_commit_success_count": int(overall.get("frontier_commit_success_count", 0)),
            "frontier_switch_count": int(overall.get("frontier_switch_count", 0)),
            "stagnation_recovery_count": int(overall.get("stagnation_recovery_count", 0)),
            "revisit_rate": float(overall.get("revisit_rate", 0.0)),
            "mean_distance_to_selected_frontier": float(overall.get("mean_distance_to_selected_frontier", 0.0)),
            "percent_steps_targeting_unknown_frontier": float(overall.get("percent_steps_targeting_unknown_frontier", 0.0)),
            "percent_steps_safe_forward_bias_active": float(overall.get("percent_steps_safe_forward_bias_active", 0.0)),
            "selected_frontier_score": float(overall.get("selected_frontier_score", 0.0)),
            "frontier_information_gain": float(overall.get("frontier_information_gain", 0.0)),
            "frontier_reachability_score": float(overall.get("frontier_reachability_score", 0.0)),
            "average_frontier_area": float(overall.get("average_frontier_area", 0.0)),
        },
        "by_difficulty": {k: v.get("aggregate", {}) for k, v in by_diff.items()},
    }
    (out_dir / "planner_diagnostics.json").write_text(json.dumps(planner_diagnostics, indent=2), encoding="utf-8")

    readme = (
        "# Phase 2D.2 Coverage Expansion\n\n"
        "This run evaluates mapper-guided navigation coverage expansion using the accepted mapper.\n\n"
        "Outputs:\n"
        "- `mapper_guided_navigation_v3_results.json`\n"
        "- `comparison_v2_vs_v3.json`\n"
        "- `coverage_by_map_and_difficulty.json`\n"
        "- `planner_diagnostics.json`\n"
        "- `plots/`\n\n"
        "Safety constraints remain hard-filtered: movement actions are collision-checked before execution.\n"
    )
