...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Phase 2C.3 - soft-gated doorway false-positive control (v4).")
    parser.add_argument("--data-dir", type=str, default="datasets/phase2_echo_mapping")
    parser.add_argument("--output-dir", type=str, default="runs/phase2_echo_mapper_v4")
...
def metrics_at_thresholds(
    ...
    if doorway_mode == "soft_gated" and ("door_prob_soft_gated" in arrays):
        door_key = "door_prob_soft_gated"
    else:
        door_key = "door_prob"
...
def save_prediction_plots(
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            door_prob = door_prob_soft_gated if use_soft_doorway_gating else door_prob_raw
            ...
            for i in range(occ_prob.shape[0]):
                ...
                axs[14].text(0.0, 0.2, f"soft_gated={int(use_soft_doorway_gating)}", fontsize=10)
                axs[15].axis("off")
...
def main() -> None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        door_prob_for_eval = val_arrays["door_prob_soft_gated"] if use_soft_doorway_gating and ("door_prob_soft_gated" in val_arrays) else val_arrays["door_prob"]
        ...
        history.append(
            {
                ...
                "val_best_thresholds": {"occupancy": best_occ_t, "wall": best_wall_t, "doorway": best_door_t, "free": best_free_t},
    ...
    selected_thresholds = {"occupancy": float(best_occ_t), "wall": float(best_wall_t), "doorway": float(best_door_t), "free": float(best_free_t)}

    val_metrics_best = metrics_at_thresholds(val_arrays, selected_thresholds, map_names, difficulty_names, doorway_mode=("soft_gated" if use_soft_doorway_gating else "raw"))
    val_metrics_no_gating = metrics_at_thresholds(val_arrays, selected_thresholds, map_names, difficulty_names, doorway_mode="raw")
    val_metrics_soft_gating = metrics_at_thresholds(val_arrays, selected_thresholds, map_names, difficulty_names, doorway_mode="soft_gated")
    ...
    validation_payload = {
        ...
        "threshold_search": {
            ...
            "all_threshold_stats": {"occupancy": occ_all, "wall": wall_all, "doorway": door_all, "free": free_all},
        },
        "val_losses": val_loss_metrics,
        "val_metrics_without_gating": val_metrics_no_gating,
        "val_metrics_with_soft_gating": val_metrics_soft_gating,
    ...
    door_prob_test_eval = test_arrays["door_prob_soft_gated"] if use_soft_doorway_gating and ("door_prob_soft_gated" in test_arrays) else test_arrays["door_prob"]
    test_thr_05 = {
        ...
        "doorway": binary_stats(door_prob_test_eval >= 0.5, test_arrays["door_t"]),
        "free": binary_stats(test_arrays["free_prob"] >= 0.5, test_arrays["free_t"]),
    }

    test_metrics_best = metrics_at_thresholds(test_arrays, selected_thresholds, map_names, difficulty_names, doorway_mode=("soft_gated" if use_soft_doorway_gating else "raw"))
    test_metrics_no_gating = metrics_at_thresholds(test_arrays, selected_thresholds, map_names, difficulty_names, doorway_mode="raw")
    test_metrics_soft_gating = metrics_at_thresholds(test_arrays, selected_thresholds, map_names, difficulty_names, doorway_mode="soft_gated")
    ...
    test_payload = {
        ...
        "test_metrics_with_soft_gating": test_metrics_soft_gating,
