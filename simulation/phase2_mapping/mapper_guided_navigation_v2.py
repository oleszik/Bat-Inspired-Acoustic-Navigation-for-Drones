...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=str, default="runs/phase2_mapper_guided_navigation_v2")
...
def find_frontier_target(
    ...
    # Prefer cells that can likely reveal new information.
    ...
    for y in range(1, ny - 1):
        for x in range(1, nx - 1):
            ...
            if min_conf < 0.40:
                # Lower confidence frontier + higher free probability + larger local confidence contrast.
                conf_gap = max(0.0, local_conf - min_conf)
                score = float(1.6 * free_prob[y, x] + 1.3 * conf_gap - 0.9 * local_conf)
...
def cast_observation_footprint(
    ...
    spread = np.linspace(-math.radians(12.0), math.radians(12.0), max(1, rays_per_sector))
    ray_angles = []
...
def run_episode(
    ...
    for step in range(max_steps):
        ...
        observed_range = 3.0 if difficulty in {"clean", "mild_noise"} else 2.8
        rays_per_sector = 11 if difficulty in {"clean", "mild_noise"} else 7
        ...
        stagnation = steps_since_frontier_progress >= 20
        if stagnation:
            stagnation_event_count += 1
            if (step % 5) == 0:
                stagnation_points.append((x, y))

        frontier = find_frontier_target(free_global, conf_global, observed, blocked=blocked_frontier_cells)
        target_angle = None
        ...
        # Early stagnation cut: prevents spending all steps in probe/turn loops.
        if step >= 260 and steps_since_frontier_progress >= 95 and coverage_now < 0.62:
            break
    ...
    elif not success and coverage < 0.70:
...
def save_episode_plot(out_dir: Path, map_name: str, difficulty: str, ep: Dict[str, object], m) -> None:
    ...
    ax[4].imshow(ep.get("wall_map_thresholded", np.zeros_like(ep["gt_wall"])), cmap="gray_r", vmin=0, vmax=1)
    ax[4].set_title(f"Thresholded Wall (t={ep.get('wall_threshold_used', 0.0):.2f})")
    ax[5].imshow(ep.get("door_map_thresholded", np.zeros_like(ep["gt_door"])), cmap="gray_r", vmin=0, vmax=1)
    ax[5].set_title(f"Thresholded Doorway (t={ep.get('doorway_threshold_used', 0.0):.2f})")
    ...
    ax[7].imshow(gt_overlay, cmap="inferno", vmin=0.0, vmax=1.0)
    ax[7].set_title("GT Wall/Doorway Reference")
