...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Phase 2D.3d mapper-guided navigation v3.3 low-coverage recovery.")
    ...
    parser.add_argument("--output-dir", type=str, default="runs/phase2_mapper_guided_navigation_v33")
    ...
    parser.add_argument("--low-coverage-recovery-enabled", action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument("--low-coverage-threshold", type=float, default=0.55)
    parser.add_argument("--coverage-gain-window-steps", type=int, default=80)
    parser.add_argument("--min-steps-before-recovery", type=int, default=250)
    parser.add_argument("--min-recovery-duration-steps", type=int, default=40)
    parser.add_argument("--max-recovery-duration-steps", type=int, default=120)
    parser.add_argument("--local-obstacle-density-threshold", type=float, default=0.20)
    parser.add_argument("--recovery-cooldown-steps", type=int, default=100)
...
def find_recovery_target(
    ...
    current_cell: Tuple[int, int],
    recent_cells: List[Tuple[int, int]],
    failed_until: Optional[Dict[Tuple[int, int], int]] = None,
    step_idx: int = 0,
) -> Tuple[Optional[Tuple[int, int]], Dict[str, float]]:
    failed_until = failed_until or {}
    ny, nx = free_prob.shape
    cx, cy = current_cell
    recent_counts = Counter(recent_cells)
    candidates = []
    for y in range(2, ny - 2):
        for x in range(2, nx - 2):
            cell = (x, y)
            if failed_until.get(cell, -1) > step_idx:
                continue
            if free_prob[y, x] < 0.50:
                continue
            patch_conf = confidence[y - 2 : y + 3, x - 2 : x + 3]
            patch_free = free_prob[y - 2 : y + 3, x - 2 : x + 3]
            patch_obs = observed[y - 2 : y + 3, x - 2 : x + 3]
            low_conf_cells = float((patch_conf < 0.48).sum())
            unknown_cells = float(np.logical_not(patch_obs).sum())
            expected_gain = low_conf_cells + 0.75 * unknown_cells
            if expected_gain <= 2.0:
                continue
            dist = float(math.hypot(x - cx, y - cy))
            if dist < 4.0:
                continue
            revisit_penalty = float(recent_counts.get(cell, 0))
            reachability = float(np.clip(0.7 * free_prob[y, x] + 0.3 * patch_free.mean(), 0.0, 1.0))
            far_bonus = min(dist / 24.0, 1.0)
            score = float(
                2.5 * expected_gain
                + 2.0 * (1.0 - float(patch_conf.mean()))
                + 1.5 * reachability
                + 1.2 * far_bonus
                - 1.6 * revisit_penalty
                - 0.02 * dist
            ...
            candidates.append(
                {
                    ...
                    "low_confidence": float(patch_conf.mean() < 0.50),
                }
            )
    if not candidates:
        return None, {
            "recovery_target_score": 0.0,
            "recovery_target_expected_gain": 0.0,
            "recovery_target_distance": 0.0,
            "recovery_target_reachability": 0.0,
            "recovery_low_confidence_target": 0.0,
        }
    candidates.sort(key=lambda item: item["score"], reverse=True)
    best = candidates[0]
    return (int(best["x"]), int(best["y"])), {
        "recovery_target_score": float(best["score"]),
        "recovery_target_expected_gain": float(best["expected_gain"]),
        "recovery_target_distance": float(best["distance"]),
        "recovery_target_reachability": float(best["reachability"]),
        "recovery_low_confidence_target": float(best["low_confidence"]),
    }
...
def run_episode(
    ...
    low_coverage_recovery_enabled: bool,
    low_coverage_threshold: float,
    coverage_gain_window_steps: int,
    min_steps_before_recovery: int,
    min_recovery_duration_steps: int,
    max_recovery_duration_steps: int,
    local_obstacle_density_threshold: float,
    ...
    recovery_target_cell: Optional[Tuple[int, int]] = None
    recovery_start_cov = 0.0
    recovery_last_cov = 0.0
    recovery_failed_targets_until: Dict[Tuple[int, int], int] = {}
    recovery_activation_count = 0
    recovery_active_steps = 0
    recovery_exit_reasons = Counter()
    recovery_coverage_gain: List[float] = []
    recovery_success_count = 0
    recovery_failed_count = 0
    recovery_target_switch_count = 0
    recovery_target_reached_count = 0
    recovery_low_confidence_target_count = 0
    recovery_failed_target_cooldown_count = 0
    low_coverage_stagnation_detected_count = 0
    recovery_safe_forward_count = 0
    recovery_probe_directed_count = 0
    recovery_turn_count = 0
    recovery_last_turn_action: Optional[str] = None
    recovery_repeat_turn_count = 0
    ...
    for step in range(max_steps):
        ...
        recovery_window_gain = 1.0
        if len(coverage_trace) > coverage_gain_window_steps:
            recovery_window_gain = float(coverage_trace[-1] - coverage_trace[-1 - coverage_gain_window_steps])
        close_sector_count = sum(1 for sector in SECTOR_NAMES if float(obs[sector]["distance"]) < 0.55)
        close_sector_density = float(close_sector_count / max(1, len(SECTOR_NAMES)))
        local_occ_density = float((occ_p > 0.62).mean())
        local_obstacle_density = float(max(close_sector_density, local_occ_density))
        doorway_like_now = bool(
            front_door > float(selected_modes["doorway_threshold"])
            and front_free > 0.45
            and front_conf < 0.66
        )
        narrow_passage_like = bool(
            min(float(obs["left"]["distance"]), float(obs["right"]["distance"])) < 0.38
            and float(obs["front"]["distance"]) > 0.35
            and front_free > 0.45
        )
        long_wall_like = bool(
            abs(float(obs["left"]["distance"]) - float(obs["right"]["distance"])) < 0.18
            and min(float(obs["left"]["distance"]), float(obs["right"]["distance"])) < 0.55
            and float(obs["front"]["distance"]) > 0.45
        )
        front_sector_density = float(
            sum(1 for sector in ["front_left", "front", "front_right"] if float(obs[sector]["distance"]) < 0.55) / 3.0
        )
        side_wall_corridor_like = bool(
            front_sector_density < 0.34
            and min(float(obs["left"]["distance"]), float(obs["right"]["distance"])) < 0.55
            and front_free > 0.42
        )
        low_coverage_stagnation = bool(
            low_coverage_recovery_enabled
            and coverage_now < low_coverage_threshold
            and len(coverage_trace) > coverage_gain_window_steps
            and recovery_window_gain < max(0.004, coverage_min_gain * 0.6)
            and step > min_steps_before_recovery
        )
        clutter_like = bool(
            local_obstacle_density >= local_obstacle_density_threshold
            and (front_sector_density >= 0.34 or local_occ_density >= 0.18)
            and not narrow_passage_like
            and not long_wall_like
            and not side_wall_corridor_like
        )
        if low_coverage_stagnation:
            low_coverage_stagnation_detected_count += 1

        if recovery_active:
            recovery_active_steps += 1
            recovery_gain = float(coverage_now - recovery_start_cov)
            recovery_age = step - recovery_start_step
            if recovery_age >= min_recovery_duration_steps and coverage_now >= low_coverage_threshold:
                recovery_exit_reasons["coverage_recovered"] += 1
                recovery_coverage_gain.append(max(0.0, recovery_gain))
                recovery_success_count += 1
                recovery_active = False
                recovery_cooldown_until = step + recovery_cooldown_steps
                recovery_target_cell = None
            elif recovery_age >= min_recovery_duration_steps and recovery_gain >= 0.025:
                recovery_exit_reasons["coverage_improved"] += 1
                recovery_coverage_gain.append(max(0.0, recovery_gain))
                recovery_success_count += 1
                recovery_active = False
                recovery_cooldown_until = step + recovery_cooldown_steps
                recovery_target_cell = None
            elif recovery_age >= max_recovery_duration_steps:
                recovery_exit_reasons["max_duration_no_gain"] += 1
                recovery_coverage_gain.append(max(0.0, recovery_gain))
                recovery_failed_count += 1
                if recovery_target_cell is not None:
                    recovery_failed_targets_until[recovery_target_cell] = step + recovery_cooldown_steps
                    recovery_failed_target_cooldown_count += 1
                recovery_active = False
                recovery_cooldown_until = step + recovery_cooldown_steps
                recovery_target_cell = None
        ...
        if (
            (not recovery_active)
            and low_coverage_stagnation
            and clutter_like
            and (step >= recovery_cooldown_until)
            and (not imminent_collision)
            and (not doorway_like_now)
            and (not narrow_passage_like)
            and (not side_wall_corridor_like)
        ):
            recovery_target_cell, recovery_stats = find_recovery_target(
                free_prob=free_global,
                confidence=conf_global,
                observed=observed,
                current_cell=current_cell,
                recent_cells=list(recent_cells),
                failed_until=recovery_failed_targets_until,
                step_idx=step,
            )
            if recovery_target_cell is not None:
                recovery_active = True
                recovery_start_step = step
                recovery_start_cov = coverage_now
                recovery_last_cov = coverage_now
                recovery_activation_count += 1
                recovery_last_turn_action = None
                recovery_repeat_turn_count = 0
                if recovery_stats.get("recovery_low_confidence_target", 0.0) > 0.0:
                    recovery_low_confidence_target_count += 1
        ...
        if recovery_active:
            if recovery_target_cell is None:
                recovery_target_cell, recovery_stats = find_recovery_target(
                    free_prob=free_global,
                    confidence=conf_global,
                    observed=observed,
                ...
                if recovery_target_cell is not None:
                    recovery_target_switch_count += 1
                    if recovery_stats.get("recovery_low_confidence_target", 0.0) > 0.0:
                        recovery_low_confidence_target_count += 1
            if recovery_target_cell is not None:
                rfx, rfy = recovery_target_cell
                if math.hypot(rfx - current_cell[0], rfy - current_cell[1]) <= 2.0:
                    recovery_target_reached_count += 1
                    recovery_failed_targets_until[recovery_target_cell] = step + max(20, recovery_cooldown_steps // 2)
                    recovery_target_cell, recovery_stats = find_recovery_target(
                        free_prob=free_global,
                        confidence=conf_global,
                        observed=observed,
                    ...
                    recovery_target_switch_count += 1
                    if recovery_target_cell is not None and recovery_stats.get("recovery_low_confidence_target", 0.0) > 0.0:
                        recovery_low_confidence_target_count += 1
                if recovery_target_cell is not None:
                    rfx, rfy = recovery_target_cell
                    rwx = (rfx + 0.5) * cell_size
                    rwy = (rfy + 0.5) * cell_size
                    target_angle = math.atan2(rwy - y, rwx - x)
                    frontier_targets.append((rwx, rwy))
        ...
        if recovery_active and recovery_target_cell is not None and target_angle is not None and not imminent_collision:
            err = wrap_angle(target_angle - heading)
            safe_probe_forward = (
                float(obs["front"]["distance"]) > 0.14
                and not predict_collision(m, x, y, heading, 0.03, 0.12, 0.05 if m.name != "cluttered_room" else 0.08)
            )
            safe_slow_forward = (
            ...
            ahead_x = x + 0.12 * math.cos(heading)
            ahead_y = y + 0.12 * math.sin(heading)
            ahead_cell = world_to_cell(ahead_x, ahead_y, cell_size, nx, ny)
            ahead_conf = float(conf_global[ahead_cell[1], ahead_cell[0]])
            forward_toward_target = bool(abs(err) <= math.radians(28.0))
            if forward_toward_target and safe_slow_forward and ahead_conf >= 0.35:
                action = "MOVE_FORWARD_SLOW"
                recovery_safe_forward_count += 1
                recovery_last_turn_action = None
                recovery_repeat_turn_count = 0
            elif forward_toward_target and safe_probe_forward:
                action = "PROBE_FORWARD"
                recovery_probe_directed_count += 1
                recovery_last_turn_action = None
                recovery_repeat_turn_count = 0
            elif abs(err) > math.radians(12.0):
                preferred_turn = "TURN_LEFT" if err > 0 else "TURN_RIGHT"
                if recovery_last_turn_action == preferred_turn:
                    recovery_repeat_turn_count += 1
                else:
                    recovery_repeat_turn_count = 0
                if recovery_repeat_turn_count >= 3 and safe_probe_forward:
                    action = "PROBE_FORWARD"
                    recovery_probe_directed_count += 1
                    recovery_last_turn_action = None
                    recovery_repeat_turn_count = 0
                else:
                    action = preferred_turn
                    recovery_turn_count += 1
                    recovery_last_turn_action = preferred_turn
            elif safe_probe_forward:
                action = "PROBE_FORWARD"
                recovery_probe_directed_count += 1
                recovery_last_turn_action = None
                recovery_repeat_turn_count = 0
    ...
    if recovery_active:
        recovery_exit_reasons["episode_end"] += 1
        recovery_coverage_gain.append(max(0.0, float(coverage_trace[-1]) - recovery_start_cov) if coverage_trace else 0.0)
        recovery_failed_count += 1
    ...
    return {
        ...
        "recovery_activation_count": int(recovery_activation_count),
        "recovery_active_steps": int(recovery_active_steps),
        "recovery_exit_reasons": {k: int(v) for k, v in recovery_exit_reasons.items()},
        "recovery_coverage_gain": float(np.mean(recovery_coverage_gain)) if recovery_coverage_gain else 0.0,
        "recovery_success_count": int(recovery_success_count),
        "recovery_failed_count": int(recovery_failed_count),
        "recovery_target_switch_count": int(recovery_target_switch_count),
        "recovery_target_reached_count": int(recovery_target_reached_count),
        "recovery_low_confidence_target_count": int(recovery_low_confidence_target_count),
        "recovery_failed_target_cooldown_count": int(recovery_failed_target_cooldown_count),
        "low_coverage_stagnation_detected_count": int(low_coverage_stagnation_detected_count),
        "recovery_safe_forward_count": int(recovery_safe_forward_count),
        "recovery_probe_directed_count": int(recovery_probe_directed_count),
        "recovery_turn_count": int(recovery_turn_count),
        "local_obstacle_density": float(local_obstacle_density) if "local_obstacle_density" in locals() else 0.0,
...
def aggregate(episodes: List[Dict[str, object]]) -> Dict[str, object]:
    ...
    for e in episodes:
        ...
        recovery_exit_reasons.update(e.get("recovery_exit_reasons", {}))  # type: ignore[arg-type]
    ...
    return {
        ...
        "recovery_activation_count": int(np.sum([e.get("recovery_activation_count", 0) for e in episodes])),
        "recovery_active_steps": int(np.sum([e.get("recovery_active_steps", 0) for e in episodes])),
        "recovery_exit_reasons": {k: int(v) for k, v in recovery_exit_reasons.items()},
        "recovery_coverage_gain": float(np.mean([e.get("recovery_coverage_gain", 0.0) for e in episodes])),
        "recovery_success_count": int(np.sum([e.get("recovery_success_count", 0) for e in episodes])),
        "recovery_failed_count": int(np.sum([e.get("recovery_failed_count", 0) for e in episodes])),
        "recovery_target_switch_count": int(np.sum([e.get("recovery_target_switch_count", 0) for e in episodes])),
        "recovery_target_reached_count": int(np.sum([e.get("recovery_target_reached_count", 0) for e in episodes])),
        "recovery_low_confidence_target_count": int(np.sum([e.get("recovery_low_confidence_target_count", 0) for e in episodes])),
        "recovery_failed_target_cooldown_count": int(np.sum([e.get("recovery_failed_target_cooldown_count", 0) for e in episodes])),
        "low_coverage_stagnation_detected_count": int(np.sum([e.get("low_coverage_stagnation_detected_count", 0) for e in episodes])),
        "recovery_safe_forward_count": int(np.sum([e.get("recovery_safe_forward_count", 0) for e in episodes])),
        "recovery_probe_directed_count": int(np.sum([e.get("recovery_probe_directed_count", 0) for e in episodes])),
        "recovery_turn_count": int(np.sum([e.get("recovery_turn_count", 0) for e in episodes])),
        "local_obstacle_density": float(np.mean([e.get("local_obstacle_density", 0.0) for e in episodes])),
...
def main() -> None:
    ...
    results: Dict[str, object] = {
        "config": {
            ...
            "low_coverage_recovery_enabled": args.low_coverage_recovery_enabled,
            "low_coverage_threshold": args.low_coverage_threshold,
            "coverage_gain_window_steps": args.coverage_gain_window_steps,
            "min_steps_before_recovery": args.min_steps_before_recovery,
            "min_recovery_duration_steps": args.min_recovery_duration_steps,
            "max_recovery_duration_steps": args.max_recovery_duration_steps,
            "local_obstacle_density_threshold": args.local_obstacle_density_threshold,
            "recovery_cooldown_steps": args.recovery_cooldown_steps,
    ...
    # Explicit historical comparison table.
    version_paths = {
        ...
        "v4_rejected": Path("runs/phase2_mapper_guided_navigation_v4/mapper_guided_navigation_results.json"),
        "v31_rejected": Path("runs/phase2_mapper_guided_navigation_v31/mapper_guided_navigation_results.json"),
        "v32_rejected": Path("runs/phase2_mapper_guided_navigation_v32/mapper_guided_navigation_results.json"),
    }
    comparison_keys = [
        "success_rate",
        "collision_rate",
    ...
    version_comp: Dict[str, object] = {"v33": {k: float(overall.get(k, 0.0)) for k in comparison_keys}}
    ...
    results["comparison_v1_v2_v3_v4_v31_v32_v33"] = version_comp
