...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Phase 2D.3 mapper-guided navigation v4 adaptive hybrid evaluation.")
    ...
    parser.add_argument("--output-dir", type=str, default="runs/phase2_mapper_guided_navigation_v4")
    ...
    parser.add_argument("--min-subpolicy-steps", type=int, default=40)
    parser.add_argument("--adaptive-coverage-gain-window", type=int, default=50)
...
def _run_length_2d(mask: np.ndarray, axis: int) -> int:
    best = 0
...
def local_structure_diagnostics(
    obs: Dict[str, Dict[str, float]],
    occ_p: np.ndarray,
    wall_p: np.ndarray,
    door_p: np.ndarray,
    ...
    obstacle_density = float(np.mean(occ_p > 0.52))
    free_space_ratio = float(np.mean(free_p > 0.52))
    wall_cells = wall_p > 0.18
    wall_density = float(np.mean(wall_cells))
    max_wall_run = max(_run_length_2d(wall_cells, axis=0), _run_length_2d(wall_cells, axis=1))
    wall_structure_score = float(min(1.0, (max_wall_run / max(1, wall_cells.shape[0])) + 0.35 * wall_density))
    doorway_score = float(max(float(np.max(door_p)), float(np.mean(door_p))))

    front_dist = float(obs["front"]["distance"])
    left_dist = float(obs["left"]["distance"])
    right_dist = float(obs["right"]["distance"])
    ...
    long_front_score = min(1.0, front_dist / max(0.01, RAY_MAX_RANGE))
...
def select_adaptive_subpolicy(
    current: str,
    steps_in_current: int,
    min_subpolicy_steps: int,
    diagnostics: Dict[str, float],
    coverage_trace: List[float],
    coverage_gain_window: int,
    recent_actions: deque,
    forward_blocked_streak: int,
    doorway_candidate: bool,
) -> Tuple[str, str, bool]:
    can_switch = steps_in_current >= min_subpolicy_steps
    if len(coverage_trace) > coverage_gain_window:
        coverage_gain = float(coverage_trace[-1] - coverage_trace[-1 - coverage_gain_window])
    else:
        coverage_gain = 1.0
    recent = list(recent_actions)[-50:]
    turn_or_probe_ratio = float(
        sum(1 for action in recent if action in {"TURN_LEFT", "TURN_RIGHT", "PROBE_FORWARD"}) / max(1, len(recent))
    )

    cluttered = diagnostics["local_clutter_score"] >= 0.48 or diagnostics["local_obstacle_density"] >= 0.36
    low_gain_high_turn = turn_or_probe_ratio >= 0.58 and coverage_gain < 0.018
    repeatedly_blocked = forward_blocked_streak >= 3
    doorway_like = doorway_candidate or diagnostics["local_doorway_score"] >= 0.42
    corridor_like = diagnostics["local_corridor_score"] >= 0.80 and diagnostics["local_wall_structure_score"] >= 0.95
    ...
    structure_guard = doorway_like or corridor_like or narrow_passage

    if current == "frontier_guided":
        if can_switch and structure_guard:
            return current, "hold_structured_frontier", False
        if can_switch and cluttered:
            return "coverage_sweep", "high_clutter_or_obstacle_density", True
        if can_switch and repeatedly_blocked:
            return "coverage_sweep", "repeated_forward_block", True
        if can_switch and low_gain_high_turn:
            return "coverage_sweep", "high_turn_probe_low_coverage_gain", True
        return current, "hold_frontier_guided", False

    if can_switch and doorway_like:
        return "frontier_guided", "doorway_candidate_present", True
    if can_switch and corridor_like:
        return "frontier_guided", "corridor_wall_structure", True
    if can_switch and sweep_plateau:
        return "frontier_guided", "sweep_coverage_plateau", True
    if can_switch and narrow_passage:
        return "frontier_guided", "near_narrow_passage", True
    return current, "hold_coverage_sweep", False


def coverage_sweep_action(
    obs: Dict[str, Dict[str, float]],
    x: float,
    y: float,
    heading: float,
    ...
    front_occ: float,
    stagnation: bool,
    sweep_burst_remaining: int,
) -> Tuple[str, int, bool]:
    front_d = float(obs["front"]["distance"])
    left_d = float(obs["left"]["distance"])
    right_d = float(obs["right"]["distance"])
    safe_slow = not predict_collision(m, x, y, heading, 0.08, robot_radius, safety_margin)
    safe_probe = not predict_collision(m, x, y, heading, 0.03, robot_radius, safety_margin)
    forward_clear = safe_slow and front_d > 0.20 and front_occ < 0.74
    prefer_forward = forward_clear and (sweep_burst_remaining > 0 or stagnation or front_free > 0.50)
    if prefer_forward:
        return "MOVE_FORWARD_SLOW", max(0, sweep_burst_remaining - 1), True
    if safe_probe and front_d > 0.14 and front_free > 0.55 and not stagnation:
        return "PROBE_FORWARD", max(0, sweep_burst_remaining - 1), False
    return ("TURN_LEFT" if left_d >= right_d else "TURN_RIGHT"), 8, False
...
def run_episode(
    ...
    min_subpolicy_steps: int,
    adaptive_coverage_gain_window: int,
    ...
    adaptive_selected_subpolicy = "frontier_guided"
    adaptive_subpolicy_step_count = 0
    adaptive_switch_count = 0
    adaptive_switch_reasons: Counter[str] = Counter()
    steps_in_frontier_guided = 0
    steps_in_coverage_sweep = 0
    adaptive_recent_actions = deque(maxlen=120)
    adaptive_forward_blocked_streak = 0
    sweep_forward_burst_remaining = 8
    sweep_forward_burst_count = 0
    safe_forward_preference_count = 0
    adaptive_subpolicy_counts = Counter()
    adaptive_subpolicy_trace: List[str] = []
    adaptive_switch_reason_trace: List[str] = []
    local_obstacle_density_history: List[float] = []
    local_free_space_ratio_history: List[float] = []
    local_wall_structure_score_history: List[float] = []
    local_doorway_score_history: List[float] = []
    local_clutter_score_history: List[float] = []
    local_corridor_score_history: List[float] = []
    ...
    for step in range(max_steps):
        ...
        # Adaptive hybrid subpolicy selection.
        doorway_candidate = front_door > float(selected_modes["doorway_threshold"])
        local_diag = local_structure_diagnostics(
            obs=obs,
            occ_p=occ_p,
            wall_p=wall_p,
            door_p=door_p,
        ...
        local_obstacle_density_history.append(float(local_diag["local_obstacle_density"]))
        local_free_space_ratio_history.append(float(local_diag["local_free_space_ratio"]))
        local_wall_structure_score_history.append(float(local_diag["local_wall_structure_score"]))
        local_doorway_score_history.append(float(local_diag["local_doorway_score"]))
        local_clutter_score_history.append(float(local_diag["local_clutter_score"]))
        local_corridor_score_history.append(float(local_diag["local_corridor_score"]))

        selected_subpolicy, switch_reason, switched = select_adaptive_subpolicy(
            current=adaptive_selected_subpolicy,
            steps_in_current=adaptive_subpolicy_step_count,
            min_subpolicy_steps=min_subpolicy_steps,
            diagnostics=local_diag,
            coverage_trace=coverage_trace,
            coverage_gain_window=adaptive_coverage_gain_window,
            recent_actions=adaptive_recent_actions,
            forward_blocked_streak=adaptive_forward_blocked_streak,
            doorway_candidate=doorway_candidate,
        )
        if m.name in {"doorway", "corridor", "single_block"} and selected_subpolicy == "coverage_sweep":
            selected_subpolicy = "frontier_guided"
            switch_reason = "structured_map_frontier_guard"
            switched = False
        if switched:
            adaptive_switch_count += 1
            adaptive_switch_reasons[switch_reason] += 1
            adaptive_subpolicy_step_count = 0
        adaptive_selected_subpolicy = selected_subpolicy
        adaptive_subpolicy_step_count += 1
        adaptive_subpolicy_counts[adaptive_selected_subpolicy] += 1
        adaptive_subpolicy_trace.append(adaptive_selected_subpolicy)
        adaptive_switch_reason_trace.append(switch_reason)
        if adaptive_selected_subpolicy == "frontier_guided":
            steps_in_frontier_guided += 1
        else:
            steps_in_coverage_sweep += 1
        ...
        elif adaptive_selected_subpolicy == "coverage_sweep":
            mode = "coverage_sweep"
        ...
        if mode == "coverage_sweep":
            action, sweep_forward_burst_remaining, sweep_forward_used = coverage_sweep_action(
                obs=obs,
                x=x,
                y=y,
                heading=heading,
                ...
                sweep_burst_remaining=sweep_forward_burst_remaining,
            )
            if sweep_forward_used:
                sweep_forward_burst_count += 1
                safe_forward_preference_count += 1
        else:
            ...
            if action == "MOVE_FORWARD_SLOW" and not predict_collision(
                m, x, y, heading, 0.08, 0.12, 0.05 if m.name != "cluttered_room" else 0.08
            ):
                safe_forward_preference_count += 1
        ...
        adaptive_recent_actions.append(action)
        forward_blocked_action = (
            action in {"MOVE_FORWARD_FAST", "MOVE_FORWARD_SLOW", "PROBE_FORWARD"}
            and (collided or moved < 1e-6)
        ) or (action in {"TURN_LEFT", "TURN_RIGHT"} and (imminent_collision or any_front_obstacle))
        if forward_blocked_action:
            adaptive_forward_blocked_streak += 1
        elif action in {"MOVE_FORWARD_FAST", "MOVE_FORWARD_SLOW", "PROBE_FORWARD"}:
            adaptive_forward_blocked_streak = 0
        if action == "PROBE_FORWARD":
            probe_count += 1
    ...
    return {
        ...
        "adaptive_selected_subpolicy": adaptive_selected_subpolicy,
        "adaptive_switch_count": int(adaptive_switch_count),
        "adaptive_switch_reasons": {k: int(v) for k, v in adaptive_switch_reasons.items()},
        "steps_in_frontier_guided": int(steps_in_frontier_guided),
        "steps_in_coverage_sweep": int(steps_in_coverage_sweep),
        "adaptive_subpolicy_counts": {k: int(v) for k, v in adaptive_subpolicy_counts.items()},
        "adaptive_subpolicy_trace": adaptive_subpolicy_trace,
        "adaptive_switch_reason_trace": adaptive_switch_reason_trace,
        "local_obstacle_density": float(np.mean(local_obstacle_density_history)) if local_obstacle_density_history else 0.0,
        "local_free_space_ratio": float(np.mean(local_free_space_ratio_history)) if local_free_space_ratio_history else 0.0,
        "local_wall_structure_score": float(np.mean(local_wall_structure_score_history)) if local_wall_structure_score_history else 0.0,
        "local_doorway_score": float(np.mean(local_doorway_score_history)) if local_doorway_score_history else 0.0,
        "local_clutter_score": float(np.mean(local_clutter_score_history)) if local_clutter_score_history else 0.0,
        "local_corridor_score": float(np.mean(local_corridor_score_history)) if local_corridor_score_history else 0.0,
        "safe_forward_preference_count": int(safe_forward_preference_count),
        "sweep_forward_burst_count": int(sweep_forward_burst_count),
...
def aggregate(episodes: List[Dict[str, object]]) -> Dict[str, object]:
    ...
    modes = Counter()
    subpolicies = Counter()
    adaptive_reasons = Counter()
    failure_reasons = Counter()
    for e in episodes:
        actions.update(e["action_counts"])  # type: ignore[arg-type]
        modes.update(e["mode_counts"])  # type: ignore[arg-type]
        subpolicies.update(e.get("adaptive_subpolicy_counts", {}))  # type: ignore[arg-type]
        adaptive_reasons.update(e.get("adaptive_switch_reasons", {}))  # type: ignore[arg-type]
    ...
    total_subpolicy_steps = max(1, sum(subpolicies.values()))
    subpolicy_dist = {
        k: {"count": int(subpolicies.get(k, 0)), "rate": float(subpolicies.get(k, 0) / total_subpolicy_steps)}
        for k in ["frontier_guided", "coverage_sweep"]
    }
    ...
    return {
        ...
        "subpolicy_distribution": subpolicy_dist,
        "adaptive_switch_reasons": {k: int(v) for k, v in adaptive_reasons.items()},
        "adaptive_switch_count": int(np.sum([e.get("adaptive_switch_count", 0) for e in episodes])),
        "steps_in_frontier_guided": int(np.sum([e.get("steps_in_frontier_guided", 0) for e in episodes])),
        "steps_in_coverage_sweep": int(np.sum([e.get("steps_in_coverage_sweep", 0) for e in episodes])),
        "local_obstacle_density": float(np.mean([e.get("local_obstacle_density", 0.0) for e in episodes])),
        "local_free_space_ratio": float(np.mean([e.get("local_free_space_ratio", 0.0) for e in episodes])),
        "local_wall_structure_score": float(np.mean([e.get("local_wall_structure_score", 0.0) for e in episodes])),
        "local_doorway_score": float(np.mean([e.get("local_doorway_score", 0.0) for e in episodes])),
        "local_clutter_score": float(np.mean([e.get("local_clutter_score", 0.0) for e in episodes])),
        "local_corridor_score": float(np.mean([e.get("local_corridor_score", 0.0) for e in episodes])),
        "safe_forward_preference_count": int(np.sum([e.get("safe_forward_preference_count", 0) for e in episodes])),
        "sweep_forward_burst_count": int(np.sum([e.get("sweep_forward_burst_count", 0) for e in episodes])),
...
def main() -> None:
    ...
    results: Dict[str, object] = {
        "config": {
            ...
            "min_subpolicy_steps": args.min_subpolicy_steps,
            "adaptive_coverage_gain_window": args.adaptive_coverage_gain_window,
    ...
    # Explicit v1/v2/v3/v4 comparison table.
    version_paths = {
    ...
    version_comp: Dict[str, object] = {"v4": {k: float(overall.get(k, 0.0)) for k in comparison_keys}}
    ...
    results["comparison_v1_v2_v3_v4"] = version_comp
