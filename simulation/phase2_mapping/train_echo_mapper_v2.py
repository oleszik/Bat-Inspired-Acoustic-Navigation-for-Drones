...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Phase 2C.1 - improved neural echo mapper (v2).")
    parser.add_argument("--data-dir", type=str, default="datasets/phase2_echo_mapping")
    parser.add_argument("--output-dir", type=str, default="runs/phase2_echo_mapper_v2")
    ...
    parser.add_argument(
        ...
        type=str,
        default="0.1,0.15,0.2,0.25,0.3,0.35,0.4,0.45,0.5,0.6,0.7",
    )
...
def metrics_at_thresholds(
    ...
    door_pred = arrays["door_prob"] >= thresholds["doorway"]
    ...
    for name in NON_DOORWAY_MAPS:
        ...
        mask_map = arrays["map_idx"] == idx
        gt_neg = arrays["door_t"][mask_map].sum(axis=1) == 0
        if gt_neg.any():
            pred_pos = door_pred[mask_map][gt_neg].any(axis=1)
            fpr[name] = float(pred_pos.mean())
        else:
            fpr[name] = 0.0
...
def save_prediction_plots(
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            door_prob = torch.sigmoid(out["doorway_logits"]).cpu().numpy().reshape(-1, patch_size, patch_size)
            ...
            for i in range(occ_prob.shape[0]):
                ...
                axs[8].imshow(door_prob[i], cmap="viridis", vmin=0, vmax=1)
                axs[8].set_title("Pred Door Prob")
                ...
                axs[13].axis("off")
                axs[13].text(0.0, 0.8, f"sample={count}", fontsize=10)
                axs[13].text(0.0, 0.6, f"door_gt_px={int(door_gt[i].sum())}", fontsize=10)
                axs[13].text(0.0, 0.4, f"door_pred_px={int(door_bin[i].sum())}", fontsize=10)
                axs[13].text(0.0, 0.2, f"occ_iou_local~", fontsize=10)
                axs[14].axis("off")
                axs[15].axis("off")
...
def run_train_epoch(
    ...
    for batch in loader:
        ...
        loss_door = 0.65 * loss_door_focal + 0.35 * loss_door_dice + doorway_hard_neg_weight * loss_door_hn
...
def main() -> None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        door_t05 = binary_stats(val_arrays["door_prob"] >= 0.5, val_arrays["door_t"])
        ...
        best_door_t, best_door_stats, _ = scan_best_threshold(val_arrays["door_prob"], val_arrays["door_t"], thresholds, metric="f1")
    ...
    best_door_t, best_door_stats, door_all = scan_best_threshold(val_arrays["door_prob"], val_arrays["door_t"], thresholds, metric="f1")
    ...
    val_thr_05 = {
        ...
        "doorway": binary_stats(val_arrays["door_prob"] >= 0.5, val_arrays["door_t"]),
    ...
    test_thr_05 = {
        ...
        "doorway": binary_stats(test_arrays["door_prob"] >= 0.5, test_arrays["door_t"]),
