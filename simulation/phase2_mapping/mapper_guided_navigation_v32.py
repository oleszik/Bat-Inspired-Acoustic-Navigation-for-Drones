...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Phase 2D.3c mapper-guided navigation v3.2 anti-oscillation smoothing.")
    ...
    parser.add_argument("--output-dir", type=str, default="runs/phase2_mapper_guided_navigation_v32")
    ...
    parser.add_argument("--min-turn-commit-steps", type=int, default=3)
...
def run_episode(
    ...
    turn_count = 0
    probe_count = 0
    turn_oscillation_count = 0
    turn_oscillation_suppressed_count = 0
    turn_direction_repeated_count = 0
    turn_direction_flipped_count = 0
    turn_commit_active_count = 0
    turn_commit_cancelled_count = 0
    last_turn_action: Optional[str] = None
    committed_turn_action: Optional[str] = None
    committed_turn_remaining = 0
    recent_turn_actions = deque(maxlen=4)
    confidence_forward_candidate_count = 0
    confidence_forward_chosen_count = 0
    confidence_forward_rejected_low_confidence_count = 0
    confidence_forward_rejected_collision_count = 0
    low_confidence_probe_preserved_count = 0
    high_confidence_probe_replaced_count = 0
    ...
    for step in range(max_steps):
        ...
        safe_slow_forward = not predict_collision(m, x, y, heading, 0.08, 0.12, 0.05 if m.name != "cluttered_room" else 0.08)
        safe_probe_forward = not predict_collision(m, x, y, heading, 0.03, 0.12, 0.05 if m.name != "cluttered_room" else 0.08)
        ahead_x = x + 0.08 * math.cos(heading)
        ahead_y = y + 0.08 * math.sin(heading)
        ahead_cx, ahead_cy = world_to_cell(ahead_x, ahead_y, cell_size, nx, ny)
        ahead_conf = float(conf_global[ahead_cy, ahead_cx])
        ahead_unobserved = not bool(observed[ahead_cy, ahead_cx])
        expected_forward_gain = float(max(0.0, 0.62 - ahead_conf) + (0.35 if ahead_unobserved else 0.0))
        local_conf_low = bool(front_conf < 0.36 or ahead_conf < 0.36 or float(conf_global.mean()) < 0.32)
        local_conf_sufficient = bool(front_conf >= 0.46 and ahead_conf >= 0.55 and float(conf_global.mean()) >= 0.36)
        forward_gain_positive = expected_forward_gain > 0.20
        forward_confidence_candidate = bool(
            safe_slow_forward
            and float(obs["front"]["distance"]) > 0.22
            and front_occ < 0.66
            and front_free > 0.52
            and (local_conf_sufficient or forward_gain_positive)
            and not local_conf_low
        )
        if action in {"MOVE_FORWARD_SLOW", "PROBE_FORWARD", "TURN_LEFT", "TURN_RIGHT"}:
            if safe_slow_forward:
                confidence_forward_candidate_count += 1
            else:
                confidence_forward_rejected_collision_count += 1
            if not forward_confidence_candidate and safe_slow_forward and local_conf_low:
                confidence_forward_rejected_low_confidence_count += 1
        if action == "PROBE_FORWARD" and local_conf_low:
            low_confidence_probe_preserved_count += 1
        elif action == "PROBE_FORWARD" and forward_confidence_candidate and local_conf_sufficient:
            action = "MOVE_FORWARD_SLOW"
            confidence_forward_chosen_count += 1
            high_confidence_probe_replaced_count += 1

        turn_oscillating_now = False
        if action in {"TURN_LEFT", "TURN_RIGHT"}:
            turn_sequence = list(recent_turn_actions) + [action]
            oscillating = len(turn_sequence) >= 4 and (
                turn_sequence[-4:] == ["TURN_LEFT", "TURN_RIGHT", "TURN_LEFT", "TURN_RIGHT"]
                or turn_sequence[-4:] == ["TURN_RIGHT", "TURN_LEFT", "TURN_RIGHT", "TURN_LEFT"]
            )
            if oscillating:
                turn_oscillating_now = True
                turn_oscillation_count += 1
                preferred_turn: Optional[str] = None
                if target_angle is not None:
                    err = wrap_angle(target_angle - heading)
                    if abs(err) > math.radians(10.0):
                        preferred_turn = "TURN_LEFT" if err > 0 else "TURN_RIGHT"
                if last_turn_action is not None and preferred_turn == last_turn_action:
                    action = last_turn_action
                    turn_oscillation_suppressed_count += 1
                elif local_conf_low and safe_probe_forward and float(obs["front"]["distance"]) > 0.14:
                    action = "PROBE_FORWARD"
                    turn_oscillation_suppressed_count += 1
                    low_confidence_probe_preserved_count += 1
                elif forward_confidence_candidate:
                    action = "MOVE_FORWARD_SLOW"
                    confidence_forward_chosen_count += 1
                    turn_oscillation_suppressed_count += 1

        if action in {"TURN_LEFT", "TURN_RIGHT"} and target_angle is not None:
            err = wrap_angle(target_angle - heading)
            preferred_turn = "TURN_LEFT" if err > 0 else "TURN_RIGHT"
            strong_geometry_change = bool(abs(err) > math.radians(55.0) and action != preferred_turn)
            if committed_turn_action is not None and committed_turn_remaining > 0:
                if action != committed_turn_action and turn_oscillating_now and not strong_geometry_change and not imminent_collision:
                    action = committed_turn_action
                    turn_commit_active_count += 1
                elif action != committed_turn_action:
                    committed_turn_action = action
                    committed_turn_remaining = min_turn_commit_steps - 1
                    if strong_geometry_change or not turn_oscillating_now:
                        turn_commit_cancelled_count += 1
                else:
                    turn_commit_active_count += 1
                    committed_turn_remaining = max(0, committed_turn_remaining - 1)
            else:
                committed_turn_action = action
                committed_turn_remaining = max(0, min_turn_commit_steps - 1)
        elif action not in {"TURN_LEFT", "TURN_RIGHT"}:
            committed_turn_action = None
            committed_turn_remaining = 0
        ...
        if action not in {"TURN_LEFT", "TURN_RIGHT"}:
            ...
            committed_turn_remaining = 0
        if action in {"TURN_LEFT", "TURN_RIGHT"}:
            if last_turn_action is not None:
                if action == last_turn_action:
                    turn_direction_repeated_count += 1
                else:
                    turn_direction_flipped_count += 1
            recent_turn_actions.append(action)
    ...
    return {
        ...
        "turn_oscillation_count": int(turn_oscillation_count),
        "turn_oscillation_suppressed_count": int(turn_oscillation_suppressed_count),
        "turn_direction_repeated_count": int(turn_direction_repeated_count),
        "turn_direction_flipped_count": int(turn_direction_flipped_count),
        "turn_commit_active_count": int(turn_commit_active_count),
        "turn_commit_cancelled_count": int(turn_commit_cancelled_count),
        "confidence_forward_candidate_count": int(confidence_forward_candidate_count),
        "confidence_forward_chosen_count": int(confidence_forward_chosen_count),
        "confidence_forward_rejected_low_confidence_count": int(confidence_forward_rejected_low_confidence_count),
        "confidence_forward_rejected_collision_count": int(confidence_forward_rejected_collision_count),
        "low_confidence_probe_preserved_count": int(low_confidence_probe_preserved_count),
        "high_confidence_probe_replaced_count": int(high_confidence_probe_replaced_count),
...
def aggregate(episodes: List[Dict[str, object]]) -> Dict[str, object]:
    ...
    return {
        ...
        "turn_oscillation_count": int(np.sum([e.get("turn_oscillation_count", 0) for e in episodes])),
        "turn_oscillation_suppressed_count": int(np.sum([e.get("turn_oscillation_suppressed_count", 0) for e in episodes])),
        "turn_direction_repeated_count": int(np.sum([e.get("turn_direction_repeated_count", 0) for e in episodes])),
        "turn_direction_flipped_count": int(np.sum([e.get("turn_direction_flipped_count", 0) for e in episodes])),
        "turn_commit_active_count": int(np.sum([e.get("turn_commit_active_count", 0) for e in episodes])),
        "turn_commit_cancelled_count": int(np.sum([e.get("turn_commit_cancelled_count", 0) for e in episodes])),
        "confidence_forward_candidate_count": int(np.sum([e.get("confidence_forward_candidate_count", 0) for e in episodes])),
        "confidence_forward_chosen_count": int(np.sum([e.get("confidence_forward_chosen_count", 0) for e in episodes])),
        "confidence_forward_rejected_low_confidence_count": int(np.sum([e.get("confidence_forward_rejected_low_confidence_count", 0) for e in episodes])),
        "confidence_forward_rejected_collision_count": int(np.sum([e.get("confidence_forward_rejected_collision_count", 0) for e in episodes])),
        "low_confidence_probe_preserved_count": int(np.sum([e.get("low_confidence_probe_preserved_count", 0) for e in episodes])),
        "high_confidence_probe_replaced_count": int(np.sum([e.get("high_confidence_probe_replaced_count", 0) for e in episodes])),
...
def main() -> None:
    ...
    results: Dict[str, object] = {
        "config": {
            ...
            "min_turn_commit_steps": args.min_turn_commit_steps,
    ...
    version_comp: Dict[str, object] = {"v32": {k: float(overall.get(k, 0.0)) for k in comparison_keys}}
    ...
    results["comparison_v1_v2_v3_v4_v31_v32"] = version_comp
