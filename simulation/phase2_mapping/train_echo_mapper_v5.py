...
class EchoMapperV3(nn.Module):
    """Phase 2C.4 multi-head mapper with doorway context gating head."""
    ...
    def __init__(
        ...
        use_doorway_context_head: bool = True,
    ) -> None:
        super().__init__()
        self.patch_size = patch_size
        self.use_visibility_head = use_visibility_head
        self.use_pose_head = use_pose_head
        self.use_doorway_context_head = use_doorway_context_head
    ...
    def forward(self, signal: torch.Tensor, meta: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        if self.doorway_context_head is not None:
            out["doorway_context_logit"] = self.doorway_context_head(feat).squeeze(1)
        return out


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Phase 2C.4 - doorway context gating for false-positive suppression (v5).")
    parser.add_argument("--data-dir", type=str, default="datasets/phase2_echo_mapping")
    parser.add_argument("--output-dir", type=str, default="runs/phase2_echo_mapper_v5")
    ...
    parser.add_argument("--doorway-negative-weight", type=float, default=1.5)
    ...
    parser.add_argument("--use-soft-doorway-gating", type=int, default=1)
    parser.add_argument("--doorway-gate-strength", type=float, default=0.5)
    parser.add_argument("--doorway-min-gate", type=float, default=0.35)
    parser.add_argument("--doorway-structure-weight", type=float, default=0.5)
    parser.add_argument("--doorway-threshold-objective", type=str, default="f1_minus_fp")
    parser.add_argument("--use-doorway-context-head", type=int, default=1)
    parser.add_argument("--context-loss-weight", type=float, default=0.5)
    parser.add_argument("--context-positive-weight", type=float, default=1.0)
    parser.add_argument("--context-hard-negative-weight", type=float, default=0.6)
    parser.add_argument("--context-threshold", type=float, default=0.5)
    parser.add_argument("--doorway-context-radius", type=int, default=2)
...
def dilate_binary_batch_np(mask: np.ndarray, radius: int) -> np.ndarray:
    """Cheap binary dilation over a batch of 2D masks using shift-union."""
    if radius <= 0:
        return mask.copy()
    b, h, w = mask.shape
    out = mask.copy()
...
def build_context_targets_from_doorway_flat(door_flat: np.ndarray, patch_size: int, radius: int) -> np.ndarray:
    """Context=1 for doorway or near-doorway patches based on GT doorway mask dilation."""
    door = door_flat.reshape(-1, patch_size, patch_size) >= 0.5
    near = dilate_binary_batch_np(door, radius=radius)
    return near.reshape(near.shape[0], -1).any(axis=1).astype(np.float32)


def context_hard_negative_penalty(
    ...
    probs = torch.sigmoid(context_logits)
    hard_neg_map = torch.zeros_like(map_index, dtype=torch.bool)
    if empty_idx >= 0:
        hard_neg_map |= map_index == empty_idx
    if single_idx >= 0:
        hard_neg_map |= map_index == single_idx
    hard_neg = (context_targets < 0.5) & hard_neg_map
    if hard_neg.any():
        return probs[hard_neg].mean()
    return torch.zeros((), device=context_logits.device)
...
def apply_doorway_structural_gating(
    ...
    # Soft structure evidence for wall-gap-wall.
    ...
    # Suppression in open-empty and isolated object-edge zones, but never hard zero.
...
def select_doorway_probability(arrays: Dict[str, np.ndarray], doorway_mode: str) -> np.ndarray:
    if doorway_mode == "raw":
        return arrays["door_prob"]
    if doorway_mode == "soft_gated":
        return arrays["door_prob_soft_gated"] if "door_prob_soft_gated" in arrays else arrays["door_prob"]
    if doorway_mode == "context_gated":
        return arrays["door_prob_context_gated"] if "door_prob_context_gated" in arrays else arrays["door_prob"]
    if doorway_mode == "final":
        return arrays["door_prob_final"] if "door_prob_final" in arrays else arrays["door_prob"]
    return arrays["door_prob"]


def context_metrics_from_arrays(
    arrays: Dict[str, np.ndarray],
    map_names: List[str],
    threshold: float,
) -> Dict[str, float]:
    if "context_prob" not in arrays or "context_t" not in arrays:
        return {}
    pred = arrays["context_prob"] >= threshold
    tgt = arrays["context_t"] >= 0.5
    stats = binary_stats(pred, tgt)

    by_map: Dict[str, float] = {}
    for map_name in ["empty_room", "single_block", "doorway"]:
        if map_name not in map_names:
            continue
        idx = map_names.index(map_name)
        mask = arrays["map_idx"] == idx
        if not np.any(mask):
            by_map[map_name] = 0.0
            continue
        if map_name == "doorway":
            by_map[map_name] = float(pred[mask].mean())
        else:
            by_map[map_name] = float(pred[mask].mean())

    return {
        "context_accuracy": stats["accuracy"],
        "context_precision": stats["precision"],
        "context_recall": stats["recall"],
        "context_f1": stats["f1"],
        "context_false_positive_rate_empty_room": by_map.get("empty_room", 0.0),
        "context_false_positive_rate_single_block": by_map.get("single_block", 0.0),
        "context_true_positive_rate_doorway_map": by_map.get("doorway", 0.0),
    }
...
def scan_best_doorway_threshold(
    ...
    for t in thresholds:
        ...
        all_stats[str(t)] = row
        # Prefer thresholds that satisfy recall floor. If none do, fallback later.
        if row["recall"] >= recall_floor and score > best_score:
            best_score = score
            best_t = float(t)
            best = row

    if not best:
        # Fallback: best combined score without recall-floor constraint.
        for t in thresholds:
            row = all_stats[str(t)]
...
def evaluate_losses_and_collect(
    ...
    use_pose_head: bool,
    use_doorway_context_head: bool,
    context_pos_weight: torch.Tensor,
    context_loss_weight: float,
    context_threshold: float,
    context_hard_negative_weight: float,
    empty_room_map_idx: int,
    single_block_map_idx: int,
    doorway_context_radius: int,
    use_soft_doorway_gating: bool,
    patch_size: int,
    doorway_gate_strength: float,
    doorway_min_gate: float,
    doorway_structure_weight: float,
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            door_patch = batch["doorway"].view(-1, 1, patch_size, patch_size)
            door_near = F.max_pool2d(
                door_patch,
                kernel_size=2 * doorway_context_radius + 1,
                stride=1,
                padding=doorway_context_radius,
            )
            context_target = (door_near.flatten(1).sum(dim=1) > 0.5).float()
            if use_doorway_context_head and "doorway_context_logit" in out:
                loss_context_bce = F.binary_cross_entropy_with_logits(
                    out["doorway_context_logit"],
                    context_target,
                    pos_weight=context_pos_weight,
                )
                loss_context_hn = context_hard_negative_penalty(
                    out["doorway_context_logit"],
                    context_target,
                    batch["map_index"],
                    empty_idx=empty_room_map_idx,
                    single_idx=single_block_map_idx,
                )
                loss_context = loss_context_bce + context_hard_negative_weight * loss_context_hn
            else:
                loss_context = torch.zeros((), device=device)
                loss_context_hn = torch.zeros((), device=device)
            ...
            sums["context"] += float(loss_context.item())
            sums["context_hard_neg"] += float(loss_context_hn.item())
            ...
            if use_doorway_context_head and "doorway_context_logit" in out:
                context_probs.append(torch.sigmoid(out["doorway_context_logit"]).cpu().numpy())
            else:
                context_probs.append(np.zeros((batch["doorway"].shape[0],), dtype=np.float32))
            ...
            context_t.append(context_target.cpu().numpy())
    ...
    arrays = {
        ...
        "context_prob": np.concatenate(context_probs, axis=0),
        ...
        "context_t": np.concatenate(context_t, axis=0) >= 0.5,
        "map_idx": np.concatenate(map_idx, axis=0),
        "diff_idx": np.concatenate(diff_idx, axis=0),
    }

    arrays["door_prob_context_gated"] = arrays["door_prob"] * arrays["context_prob"][:, None]

    if use_soft_doorway_gating:
        arrays["door_prob_soft_gated"] = apply_doorway_structural_gating(
            door_prob_2d=arrays["door_prob"],
            wall_prob_2d=arrays["wall_prob"],
            free_prob_2d=arrays["free_prob"],
            occ_prob_2d=arrays["occ_prob"],
            patch_size=patch_size,
            gate_strength=doorway_gate_strength,
            min_gate=doorway_min_gate,
            structure_weight=doorway_structure_weight,
        )
        arrays["door_prob_final"] = arrays["door_prob_soft_gated"] * arrays["context_prob"][:, None]
    else:
        arrays["door_prob_final"] = arrays["door_prob_context_gated"]
...
def metrics_at_thresholds(
    ...
    door_prob = select_doorway_probability(arrays, doorway_mode=doorway_mode)
    ...
    if "context_prob" in arrays and "context_t" in arrays:
        out["context_metrics"] = context_metrics_from_arrays(
            arrays=arrays,
            map_names=map_names,
            threshold=0.5,
        )
...
def save_prediction_plots(
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            if use_doorway_context_head and "doorway_context_logit" in out:
                context_prob = torch.sigmoid(out["doorway_context_logit"]).cpu().numpy()
            else:
                context_prob = np.ones((door_prob_raw.shape[0],), dtype=np.float32)
            door_prob_soft_gated = apply_doorway_structural_gating(
            ...
            door_prob_context = door_prob_raw * context_prob[:, None, None]
            base_for_final = door_prob_soft_gated if use_soft_doorway_gating else door_prob_raw
            door_prob_final = base_for_final * context_prob[:, None, None]

            if doorway_prediction_mode == "raw":
                door_prob = door_prob_raw
            elif doorway_prediction_mode == "soft_gated":
                door_prob = door_prob_soft_gated
            elif doorway_prediction_mode == "context_gated":
                door_prob = door_prob_context
            else:
                door_prob = door_prob_final
            ...
            for i in range(occ_prob.shape[0]):
                ...
                axs[13].imshow(door_prob_soft_gated[i], cmap="viridis", vmin=0, vmax=1)
                axs[13].set_title("Pred Door Soft-Gated")
                axs[14].axis("off")
                ...
                axs[14].text(0.0, 0.2, f"context_p={context_prob[i]:.3f}", fontsize=10)
                axs[15].imshow(door_prob_final[i], cmap="viridis", vmin=0, vmax=1)
                axs[15].set_title(f"Pred Door Final ({doorway_prediction_mode})")
...
def run_train_epoch(
    ...
    for batch in loader:
        ...
        if use_doorway_context_head and "doorway_context_logit" in out:
            loss_context_bce = F.binary_cross_entropy_with_logits(
                out["doorway_context_logit"], context_target, pos_weight=context_pos_weight
            )
            loss_context_hn = context_hard_negative_penalty(
                out["doorway_context_logit"],
                context_target,
...
def main() -> None:
    ...
    use_soft_doorway_gating = bool(args.use_soft_doorway_gating)
    use_doorway_context_head = bool(args.use_doorway_context_head)
    if use_soft_doorway_gating and use_doorway_context_head:
        doorway_prediction_mode = "final"
    elif use_soft_doorway_gating:
        doorway_prediction_mode = "soft_gated"
    elif use_doorway_context_head:
        doorway_prediction_mode = "context_gated"
    else:
        doorway_prediction_mode = "raw"
    ...
    doorway_map_idx = map_names.index("doorway") if "doorway" in map_names else -1
    empty_room_map_idx = map_names.index("empty_room") if "empty_room" in map_names else -1
    single_block_map_idx = map_names.index("single_block") if "single_block" in map_names else -1

    door_pos = (train_ds.doorway.reshape(len(train_ds), -1).sum(axis=1) > 0).astype(np.float32)
    context_pos = build_context_targets_from_doorway_flat(
        train_ds.doorway,
        patch_size=args.patch_size,
        radius=args.doorway_context_radius,
    )
    ...
    context_ratio = float(context_pos.mean())
    ...
    context_pos_weight_val = compute_pos_weight(context_ratio, min_w=1.0, max_w=40.0) * float(args.context_positive_weight)
    ...
    context_pos_weight = torch.tensor([context_pos_weight_val], dtype=torch.float32, device=device)
    ...
    model = EchoMapperV3(
        ...
        use_doorway_context_head=use_doorway_context_head,
    ).to(device)
    ...
    config = {
        ...
        "context_hard_negative_weight": args.context_hard_negative_weight,
        "doorway_context_radius": args.doorway_context_radius,
        ...
        "doorway_recall_floor": args.doorway_recall_floor,
        "doorway_threshold_objective": args.doorway_threshold_objective,
        "use_soft_doorway_gating": use_soft_doorway_gating,
        "doorway_prediction_mode": doorway_prediction_mode,
        "doorway_gate_strength": args.doorway_gate_strength,
        "doorway_min_gate": args.doorway_min_gate,
        "doorway_structure_weight": args.doorway_structure_weight,
        "threshold_candidates": thresholds,
        "wall_positive_ratio": wall_ratio,
        "doorway_positive_ratio": door_ratio,
        "context_positive_ratio": context_ratio,
        "wall_pos_weight": wall_pos_weight_val,
        "door_pos_weight": door_pos_weight_val,
        "context_pos_weight": context_pos_weight_val,
    }
    ...
    for epoch in range(1, args.epochs + 1):
        train_loss = run_train_epoch(
            ...
            use_pose_head=args.use_pose_head,
            use_doorway_context_head=use_doorway_context_head,
            context_pos_weight=context_pos_weight,
            context_loss_weight=args.context_loss_weight,
            context_hard_negative_weight=args.context_hard_negative_weight,
            empty_room_map_idx=empty_room_map_idx,
            single_block_map_idx=single_block_map_idx,
            doorway_context_radius=args.doorway_context_radius,
            patch_size=args.patch_size,
        )
        ...
        door_prob_for_eval = select_doorway_probability(val_arrays, doorway_mode=doorway_prediction_mode)
        ...
        best_occ_t, best_occ_stats, _ = scan_best_threshold(
            val_arrays["occ_prob"], val_arrays["occ_t"], thresholds, metric="iou"
        )
        best_wall_t, best_wall_stats, _ = scan_best_threshold(
            val_arrays["wall_prob"], val_arrays["wall_t"], thresholds, metric="f1"
        )
        best_door_t, best_door_stats, _ = scan_best_doorway_threshold(
            ...
            objective=args.doorway_threshold_objective,
        )
        best_free_t, best_free_stats, _ = scan_best_threshold(
            val_arrays["free_prob"], val_arrays["free_t"], thresholds, metric="accuracy"
        )
        ...
        history.append(
            {
                ...
                "doorway_mode": doorway_prediction_mode,
                "val_threshold_0p5": {
                ...
                "context_metrics": context_metrics_from_arrays(
                    arrays=val_arrays,
                    map_names=map_names,
    ...
    _, val_arrays, val_loss_metrics = evaluate_losses_and_collect(
        model=best_model,
        loader=val_loader,
        device=device,
        wall_pos_weight=wall_pos_weight,
    ...
    best_occ_t, best_occ_stats, occ_all = scan_best_threshold(
        val_arrays["occ_prob"], val_arrays["occ_t"], thresholds, metric="iou"
    )
    best_wall_t, best_wall_stats, wall_all = scan_best_threshold(
        val_arrays["wall_prob"], val_arrays["wall_t"], thresholds, metric="f1"
    )
    door_prob_for_eval = select_doorway_probability(val_arrays, doorway_mode=doorway_prediction_mode)
    ...
    best_free_t, best_free_stats, free_all = scan_best_threshold(
        val_arrays["free_prob"], val_arrays["free_t"], thresholds, metric="accuracy"
    )
    ...
    val_thr_05 = {
        "raw": binary_stats(select_doorway_probability(val_arrays, "raw") >= 0.5, val_arrays["door_t"]),
        "soft_gated": binary_stats(select_doorway_probability(val_arrays, "soft_gated") >= 0.5, val_arrays["door_t"]),
        "context_gated": binary_stats(select_doorway_probability(val_arrays, "context_gated") >= 0.5, val_arrays["door_t"]),
        "final": binary_stats(select_doorway_probability(val_arrays, "final") >= 0.5, val_arrays["door_t"]),
    }

    val_metrics_raw = metrics_at_thresholds(val_arrays, selected_thresholds, map_names, difficulty_names, doorway_mode="raw")
    val_metrics_soft = metrics_at_thresholds(val_arrays, selected_thresholds, map_names, difficulty_names, doorway_mode="soft_gated")
    val_metrics_context = metrics_at_thresholds(val_arrays, selected_thresholds, map_names, difficulty_names, doorway_mode="context_gated")
    val_metrics_final = metrics_at_thresholds(val_arrays, selected_thresholds, map_names, difficulty_names, doorway_mode="final")
    val_metrics_selected = metrics_at_thresholds(
        val_arrays,
        selected_thresholds,
        map_names,
        difficulty_names,
    ...
    val_metrics_selected["pose_correction_rmse"] = val_loss_metrics["pose_correction_rmse"]

    validation_payload = {
        "best_epoch": best_epoch,
        "best_validation_loss": best_val_loss,
        "doorway_prediction_mode": doorway_prediction_mode,
        "context_diagnostics": context_metrics_from_arrays(
            arrays=val_arrays,
            map_names=map_names,
            threshold=args.context_threshold,
        ),
        ...
        "val_metrics_soft_gated": val_metrics_soft,
        "val_metrics_context_gated": val_metrics_context,
        "val_metrics_final": val_metrics_final,
        "val_metrics_at_selected_mode": val_metrics_selected,
    }
    ...
    test_thr_05 = {
        "raw": binary_stats(select_doorway_probability(test_arrays, "raw") >= 0.5, test_arrays["door_t"]),
        "soft_gated": binary_stats(select_doorway_probability(test_arrays, "soft_gated") >= 0.5, test_arrays["door_t"]),
        "context_gated": binary_stats(select_doorway_probability(test_arrays, "context_gated") >= 0.5, test_arrays["door_t"]),
        "final": binary_stats(select_doorway_probability(test_arrays, "final") >= 0.5, test_arrays["door_t"]),
    }

    test_metrics_raw = metrics_at_thresholds(test_arrays, selected_thresholds, map_names, difficulty_names, doorway_mode="raw")
    test_metrics_soft = metrics_at_thresholds(test_arrays, selected_thresholds, map_names, difficulty_names, doorway_mode="soft_gated")
    test_metrics_context = metrics_at_thresholds(test_arrays, selected_thresholds, map_names, difficulty_names, doorway_mode="context_gated")
    test_metrics_final = metrics_at_thresholds(test_arrays, selected_thresholds, map_names, difficulty_names, doorway_mode="final")
    test_metrics_selected = metrics_at_thresholds(
        test_arrays,
        selected_thresholds,
        map_names,
        difficulty_names,
    ...
    test_metrics_selected["pose_correction_rmse"] = test_loss_metrics["pose_correction_rmse"]
    ...
    test_payload = {
        ...
        "test_metrics_soft_gated": test_metrics_soft,
        "test_metrics_context_gated": test_metrics_context,
        "test_metrics_final": test_metrics_final,
        "test_metrics_at_threshold_0.5": test_thr_05,
        "test_metrics_at_selected_mode": test_metrics_selected,
    }
    ...
    m = test_metrics_selected["overall"]
    print(
        f"Test@selected-mode({doorway_prediction_mode}) occ_iou={m['occupancy_iou']:.4f}, "
        f"wall_f1={m['wall_f1']:.4f}, "
