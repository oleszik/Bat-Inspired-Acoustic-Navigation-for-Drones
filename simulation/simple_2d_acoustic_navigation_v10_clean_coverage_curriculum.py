"""
Simple 2D acoustic navigation simulator v10 (clean-to-noisy curriculum).

This version separates planner quality from acoustic difficulty by evaluating
the same exploration policy under multiple difficulty presets.
"""
...
RESULT_JSON = RESULT_DIR / "simple_2d_acoustic_nav_v10_clean_coverage_curriculum_results.json"
RESULT_ACTION_PNG = RESULT_DIR / "simple_2d_acoustic_nav_v10_clean_coverage_curriculum_action_distribution.png"
RESULT_PATHS_PNG = RESULT_DIR / "simple_2d_acoustic_nav_v10_clean_coverage_curriculum_paths.png"
RESULT_COVERAGE_PNG = RESULT_DIR / "simple_2d_acoustic_nav_v10_clean_coverage_curriculum_coverage_maps.png"
RESULT_DIFF_SUMMARY_PNG = RESULT_DIR / "simple_2d_acoustic_nav_v10_clean_coverage_curriculum_difficulty_summary.png"
...
# v10 sensing defaults (clean/mild/medium).
...
def select_frontier_cluster(
    ...
    for cid, comp in clusters.items():
        ...
        score = 4.0 * cluster_unobs + 2.0 * cluster_size + 1.5 * d_recent - 0.6 * d_agent - blocked_penalty
        if score > best_score:
            best_score = score
            best_cid = cid
            best_target = rep
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run v10 clean-to-noisy exploration curriculum.")
...
def main() -> None:
    ...
    if "clean" in results_by_diff:
        ...
        for m in maps:
            row = {
                ...
                "v10_clean_observed_coverage": clean_maps[m.name]["mean_observed_coverage_rate"],
                "v9_visited_coverage": None,
                "v10_clean_visited_coverage": clean_maps[m.name]["mean_visited_coverage_rate"],
                "v9_collision_rate": None,
                "v10_clean_collision_rate": clean_maps[m.name]["collision_rate"],
                "v9_timeout_rate": None,
                "v10_clean_timeout_rate": clean_maps[m.name]["timeout_rate"],
                "v9_revisit_ratio": None,
                "v10_clean_revisit_ratio": clean_maps[m.name]["mean_revisit_ratio"],
                "v9_path_length": None,
                "v10_clean_path_length": clean_maps[m.name]["mean_path_length_m"],
            }
    ...
    final_status = "accepted_clean" if accepted_clean else "needs_more_tuning"
    if accepted_clean and accepted_mild:
        final_status = "accepted_mild"
